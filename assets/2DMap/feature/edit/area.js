 ...
 */
export function convertStyleToSymbol(symbol, style) {
  let borderStyle = style.borderStyle || style.borderstyle || '1' // 边框样式
  let borderWidth = style.borderWidth || style.borderwidth || 1 // 边框宽度
  let borderColor = style.borderColor || style.bordercolor || '#FF0000' // 边框颜色
  let borderTransparency = style.borderTransparency || style.bodertransparency || 40 // 边框透明度
  let fillColor = style.fillColor || style.fillcolor || '#FF0000' // 填充颜色
  let fillColorTransparency = style.fillColorTransparency || style.fillcolortransparency || style.fillTransparency || 40 // 填充透明度
  let font = style.font // 字体
  let fontColor = style.fontColor || style.fontcolor || '#000000' // 字体颜色
  let fontSize = style.fontSize || style.fontsize || 12 // 字体大小
 ...
 * @returns
 */
export function transformColor(color, opcity) {
  if (color) {
