...
function stringLineToWkt(str) {
  ...
  let ele =
    parseFloat(points[0]) + ' ' + parseFloat(points[1]) + ',' + parseFloat(points[2]) + ' ' + parseFloat(points[3])
...
export function getCenterByPolygonFeature(coords) {
  let x = 0
  let y = 0
  if (coords.length > 1) {
    coords = [coords]
  ...
  return [x / coords[0].length, y / coords[0].length]
}
