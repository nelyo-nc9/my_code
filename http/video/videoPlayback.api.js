...
// 国标设备前端回放的查询和普通的查询 测试接口
export const postFrontdevSearch = (body) => {
  const params = {
...
// 日历上有没有录像的查询
export const QUERY_RECORD_CALENDAR = (body) => post(
  {
...
// 事件记录
export const eventRecordApi = (params) => post({
...
// 新中心回放上墙
export const newVodOpen = (obj) => {
  return post({
    url: 'tvwall/v1/wall/centerplay',
    body: obj
  })
}

// 新前端回放上墙
export const newFrontVod = (obj) => {
  return post({
    url: 'tvwall/v1/wall/frontplay',
    body: obj
  })
}

// 新上墙控制
export const newVodCtrl = (obj) => {
  return post({
    url: 'tvwall/v1/wall/streamcontrol',
    body: obj
  })
}

// 获取收藏夹列表
export const getFavoriteListApi = (id) => get({
  url: 'video/v1/favorites',
  query: id
})

// 新建收藏夹
export const createFavoriteApi = (param) => {
  return post({
    url: '/api/v1/video/favorite',
    body: param
  })
}

// 机构状态
// {"queryList":[{"moduleId": "4", "childModuleId": "1", "dictId": "1"}]}
// 机构类别
// {"queryList":[{"moduleId": "4", "childModuleId": "1", "dictId": "2"}]}
// 重点部位
// {"queryList":[{"moduleId": "4", "childModuleId": "1", "dictId": "3"}]}

export const queryDataDictBasicInfoApi = (param) => {
  return post({
