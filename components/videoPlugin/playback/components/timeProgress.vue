<template>
  <div class="video-timeline" id="timeLine" :style="{height:'200px'}">
    ...
    <div class="one-stage">
      <div class="timeline-stage-top">
        <div class="stage-top-left" v-if="configuration.isAsyncType">
          <span v-for="(item,i) in typeData" @click="clickType(item)" :class="defaultType == item.label ? 'active switch':'switch'" :key="i">{{item.value}}</span>
        </div>
        <div class="stage-top-right" v-if="configuration.isNeedStepType">
          <div class="period">
            <i class="icon iconfont" :class="[ step >= steps.length - 1? 'disabled' : '']" @click="intervalChange(1)">+</i>
            <span>{{intervalArr[step].label}}</span>
            <i class="icon iconfont" :class="[step === 0  ? 'disabled' : '']" @click="intervalChange(0)">-</i>
          </div>
        </div>
        <div class="calendar" v-if="configuration.isNeedCenterTime" :style="{width:configuration.ischannelName ? 'calc(100% - 90px)':'100%'}">
      ...
      <div ref="timeline-stage-bottom" class="timeline-stage-bottom" @mousedown="timelineMouseDown" @mousemove="timelineMousemove" @mouseup="timelineMouseup" @mouseleave="timelineMouseup">
        <span class="leftSpan mt16" v-if="configuration.ischannelName">通道名称</span>
        <!-- 刻度条以及时间轴 -->
        <div class="timeline-scale" style="width: 100%;">
          <div :style="{width:configuration.ischannelName ? 'calc(100% - 90px)':'100%','margin-left':configuration.ischannelName ? '90px':0 ,overflow: 'hidden'}" class="timeLine" ref="timeLine">
            <div class="time-list" :style="{width: (timeWidth * 25 / 12) + 'px',marginLeft: offsetLeft+timeWidth/60 *1.4 + 'px'}">
              <div class="item" style="display:inline-block" :key="i" :style="{width: timeWidth/12 + 'px'}" v-for="(item, i) in thumbTimeList">{{item.time}}</div>
            </div>
            <div class="bg-line" :style="{left: offsetLeft + timeWidth/24 + 'px'}">
              ...
              <div class="scaleBox" style="width: 100%;height:10px;overflow: hidden;float: left;">
                <div class="scaleInbox" :style="{width: (timeWidth * 25 / 12) + 'px',marginLeft: offsetLeft - timeWidth/60*1.5 + 'px'}">
                  <div class="scale" :class="{'center': i%5 === 0}" :key="i" :style="{width: timeWidth/60 + 'px'}" v-for="i in 125"></div>
                </div>
              </div>

              <div class="pointer" :style="{left:configuration.ischannelName ? 'calc(50% + 44px)':'50%'}">
        ...
        <bs-scroll class="segs" style=" height: calc(100% - 40px)">
          <div v-for="(option, i) in recordOptions" :key="i" @click="clickOption(option.index)" v-show="option.isShow" :class="activeIndex === option.index ? 'active timeline-video-segments':'timeline-video-segments'" @mouseleave="leaveOption" @mouseup="upOption">
            <span style="z-index:10" :title="option.channelName" class="leftSpan" v-if="configuration.ischannelName">{{option.channelName}}</span>
            <div class="segments" :style="{width:configuration.ischannelName ? 'calc(100% - 90px)':'100%','margin-left':configuration.ischannelName ? '88px':0}" @mousedown.stop="()=>{}" @mouseup.stop="()=>{}">
              <span v-if="shearDownload &&activeIndex === i " class="time" :style="{left: `calc(${shearSectionConfiguration.left}% - 8px)`}">{{`剪切段:${shearSectionConfiguration.startTime} - ${shearSectionConfiguration.endTime}`}}</span>
              <div v-if="shearDownload  &&activeIndex === i" @mousedown="mousedownCentre($event)" @mouseup="mouseupCentre($event)" class="shear-section" :style="{left: `${shearSectionConfiguration.left}%`, width: `${shearSectionConfiguration.width}px`, zIndex: 10}">
                <span @mousedown="mousedownShearSection($event, 'left')" @mouseup="mouseupShearSection($event, 'left')" class="left"></span>
                <span @mousedown="mousedownShearSection($event, 'right')" @mouseup="mouseupShearSection($event,'right')" class="right"></span>
              </div>
              <div @mousemove="lineHover($event, option,i)" @mouseleave="leaveLine" class="segments-son" v-for="(item, index) in option.eventOptions" segments="segments" :index="item.index" :key="index+10" :style="{width: item.eventWidth + 'px', left: item.eventPosition + 'px', backgroundColor: '#FF0000',}"></div>
              <div @mousemove="lineHover($event, option,i)" @mouseleave="leaveLine" class="segments-son" v-for="(item, index) in option.timedOptions" segments="segments" :index="item.index" :key="index + 20" :style="{width: item.timedWidth + 'px', left: item.timedPosition + 'px', backgroundColor: '#32E285',}"></div>
              <div @mousemove="lineHover($event, option,i)" @mouseleave="leaveLine" class="segments-son" v-for="(item, index) in option.manualOptions" segments="segments" :index="item.index" :key="index + 40" :style="{width: item.manualWidth + 'px', left: item.manualPosition + 'px', backgroundColor: '#FF6C00',}"></div>
              <div @mousemove="lineHover($event, option,i)" @mouseleave="leaveLine" class="segments-son" v-for="(item, index) in option.intelOptions" segments="segments" :index="item.index " :key="index+ 30" :style="{width: item.intelWidth + 'px', left: item.intelPosition + 'px', backgroundColor: '#0088FF',}"></div>
            </div>
          </div>
          <!--缩略图框-->
          <div :style="{width:configuration.ischannelName ? 'calc(100% - 90px)':'100%','margin-left':configuration.ischannelName ? '88px':0,overflow: 'hidden',position: 'absolute',top:'22px'}" v-if="defaultScreen == 1 && showP && isCenter" class="thumbTime">
            <div class="time-list" :style="{width: (timeWidth * 25 / 12) + 'px',marginLeft:offsetLeft+'px','border-bottom':'1px solid #333'}">
              <div class :key="iImg" :style="{width: timeWidth/12 + 'px',height:'65px',border:'1px solid #EBEBEB', background: '#EBEBEB',float:'left'}" v-for="(itemImg, iImg) in thumbTimeList" @click="thumbClick(itemImg)" @mousemove="thumbHover($event, itemImg, iImg)" @mouseleave="leaveThumb">
...
<script>
import timelineCalc from '../common/timelineCalc'
...
export default {
  ...
  props: {
    configuration: {
      ...
      default: () => {
        return {
          isAsyncType: true, // 同步异步切换按钮
          isNeedCenterTime: true, // 中心时间
          isNeedStepType: true, // 切换时间周期
          ischannelName: true
        }
      }
    },
  ...
  data() {
    return {
      ...
      everyW: 0,
      eventType: [
        { name: '事件录像', color: '#FF0000' },
        { name: '定时录像', color: '#32E285' },
        { name: '手工录像', color: '#FF6C00' },
        { name: '智能录像', color: '#0088FF' }
      ],
      direction: ''
    }
  },
  ...
  methods: {
      ...
      if (this.isCenter) {
        ...
        obj.eventList.forEach((v, i) => {
          tempListobj.eventList.push({
            endTime: v.evtTblInfo.endTime,
            startTime: v.evtTblInfo.startTime,
            strmInfo: v.strmInfo,
            evtTblInfo: v.evtTblInfo
          })
        })
    ...
    openLine(obj) {
      if (this.recordOptions.length >= this.defaultScreen) {
        ...
        obj.isShow = true
        var catchedTimelineInfo = this.catchCalcTimeline(obj.eventList, this.centerTime, this.steps[this.step])
        obj.eventVideo = catchedTimelineInfo.eventVideo
        obj.manualVideo = catchedTimelineInfo.manualVideo
        obj.timedVideo = catchedTimelineInfo.timedVideo
        obj.intelVideo = catchedTimelineInfo.intelVideo
        ...
        if (this.isCenter) {
          ...
          this.initTimeObj.startTime = obj.eventList[0].sTime
          this.initTimeObj.endTime = obj.eventList[0].eTime
        }
      }
    },
    // 设置当前的时间段
    ...
    setCurTime(curTime, index) {
      if (this.liData[index] && this.liData[index].eventList.length != 0) {
        this.activeIndex = index
        this.initTimeObj.startTime = curTime
        this.initTimeObj.endTime =
    ...
    // 重新赋值recordsOptions
    reRecordOptions() {
      this.liData = this.sortByKey(this.liData, 'index')
    ...
    timelineMouseup(e) {
      this.canMove = false
      if (this.isMoved) {
        this.$emit('ondrag', this.centerTime)
        this.canUse = true
        setTimeout(() => {
          this.isMoved = false
    ...
    isGetTumb(time, index) {
      ...
      function compare(d) {
        for (let i = 0; i < d.length; i++) {
          if (time / 1000 > d[i].startTime || (d[i].sTime - step && time / 1000 < d[i].endTime) || d[i].eTime + step) {
            return true
          }
        }
        return false
