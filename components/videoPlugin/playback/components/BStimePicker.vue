<template>
  <div class="child" :style="{width:width + 'px', height:height + 'px'}" v-bind:class="{'disabled': disabled}">
    <div class="timeInput">
      <input type="text" class="timeInputShow test1" name="HH" :disabled='disabled' :style="{width: inputWidth + 'px'}" v-model="getHours" v-bind:id="hoursName" @focus.stop="onHoursFocus('getHours')" v-on:blur="onHoursChange" v-bind:class="{activeHours: isActiveHours}">
      <span class="timeInputOnly" readOnly=true :style="{width: spanWidth + 'px'}" value=":">
        <p>:</p>
      </span>
      <input type="text" class="timeInputShow test2" :style="{width: inputWidth + 'px'}" name="MM" :disabled='disabled' v-model="getMinute" v-bind:id="minuteName" @focus.stop="onMinuteFocus('getMinute')" v-on:blur="onMinuteChange" v-bind:class="{activeMinute: isActiveMinute}">
      <span class="timeInputOnly " readOnly=true :style="{width: spanWidth + 'px'}" value=": ">
        <p>:</p>
      </span>
      <input type="text" class="timeInputShow test3" :style="{width: inputWidth + 'px'}" name="SS " :disabled='disabled' v-model="getSecond" v-bind:id="secondName" @focus.stop="onSecondFocus('getSecond')" v-on:blur="onSecondChange" v-bind:class="{activeSecond: isActiveSecond}">
    </div>
    <div class="computedButton ">
      <button type="button " class="add icon iconfont icon-arrow-up2" v-on:click.prevent="add" v-on:mousedown.prevent="londadd" :disabled='disabled' v-bind:class="{'disabled': disabled}" v-on:mouseup="stop"></button>
      <button type="button " class="down icon iconfont icon-arrow-down2" v-on:click.prevent="down" v-on:mousedown.prevent="londdown" :disabled='disabled' v-bind:class="{'disabled': disabled}" v-on:mouseup="stop"></button>
    </div>
  </div>
</template>
