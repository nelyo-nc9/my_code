...
<script>
import { QUERY_RECORD_CALENDAR } from '@http/video/videoPlayback.api.js'
...
export default {
  ...
  methods: {
    ...
    pick: function(date) {
      ...
      this.date = date
      const test = this.formatDate(date.getFullYear(), date.getMonth() + 1, date.getDate())
  ...
  created() {
    ...
    // this.$moment.locale(this.state)
    this.week = this.$moment.weekdaysMin()
