  ...
  <div class="video-timeline"
    ...
    <div class="one-stage">
      ...
      <div ref="timeline-stage-bottom"
        ...
        <span class="leftSpan mt16"
              v-if="configuration.ischannelName">通道名称</span>
        <!-- 刻度条以及时间轴 -->
        <div class="timeline-scale"
             style="width: 100%;">
          <div :style="{width:configuration.ischannelName ? 'calc(100% - 90px)':'100%','margin-left':configuration.ischannelName ? '90px':0 ,overflow: 'hidden'}"
               class="timeLine"
               ref="timeLine">
            <div class="time-list"
              ...
              <div class="item"
                   ...
                   v-for="(item, i) in thumbTimeList">{{item.time}}</div>
            </div>
            <div class="bg-line"
                 :style="{left: offsetLeft + timeWidth/24 + 'px'}">
              <!-- 绘制刻度 -->
              <div class="scaleBox"
                   style="width: 100%;height:10px;overflow: hidden;float: left;">
                <div class="scaleInbox"
              ...
              <div class="pointer"
                   :style="{left:configuration.ischannelName ? 'calc(50% + 44px)':'50%'}">
        ...
        <!-- 进度条可滑动区域 -->
        <bs-scroll class="segs"
                   style=" height: calc(100% - 40px)">
          <div v-for="(option, i) in recordOptions"
               :key="i"
               @click="clickOption(option.index)"
               v-show="option.isShow"
               :class="full ? activeIndex === option.index ? 'timeline-video-segments':'full timeline-video-segments':activeIndex === option.index ? '  timeline-video-segments':'timeline-video-segments'"
               @mouseleave="leaveOption"
               @mouseup="upOption">
            <span style="z-index:10"
                  :title="option.channelName"
                  class="leftSpan"
                  v-if="configuration.ischannelName">{{option.channelName}}</span>
            <div class="segments"
                 :style="{width:configuration.ischannelName ? 'calc(100% - 90px)':'100%','margin-left':configuration.ischannelName ? '88px':0}"
                 @mousedown.stop="()=>{}"
                 @mouseup.stop="()=>{}">
              <span v-if="shearDownload &&activeIndex === i "
                    class="time"
                    :style="{left: `calc(${shearSectionConfiguration.left}% - 8px)`}">{{`剪切段:${shearSectionConfiguration.startTime} - ${shearSectionConfiguration.endTime}`}}</span>
              <div v-if="shearDownload  &&activeIndex === i"
                ...
                <span @mousedown="mousedownShearSection($event, 'left')"
                      @mouseup="mouseupShearSection($event, 'left')"
                      class="left"></span>
                <span @mousedown="mousedownShearSection($event, 'right')"
                      @mouseup="mouseupShearSection($event,'right')"
                      class="right"></span>
              </div>
              <div @mousemove="lineHover($event, option,i)"
                   ...
                   :index="item.index"
                   :key="index+10"
                   :style="{width: item.eventWidth + 'px', left: item.eventPosition + 'px', backgroundColor: '#FF0000',}"></div>
              <div @mousemove="lineHover($event, option,i)"
                   @mouseleave="leaveLine"
                   class="segments-son"
                   ...
                   :index="item.index"
                   :key="index + 20"
                   :style="{width: item.timedWidth + 'px', left: item.timedPosition + 'px', backgroundColor: '#32E285',}"></div>
              <div @mousemove="lineHover($event, option,i)"
                   @mouseleave="leaveLine"
                   class="segments-son"
                   ...
                   :index="item.index"
                   :key="index + 40"
                   :style="{width: item.manualWidth + 'px', left: item.manualPosition + 'px', backgroundColor: '#FF6C00',}"></div>
              <div @mousemove="lineHover($event, option,i)"
                   @mouseleave="leaveLine"
                   class="segments-son"
                   ...
                   :index="item.index "
                   :key="index+ 30"
                   :style="{width: item.intelWidth + 'px', left: item.intelPosition + 'px', backgroundColor: '#0088FF',}"></div>
            </div>
          </div>
          <!--缩略图框-->
          <div :style="{width:configuration.ischannelName ? 'calc(100% - 90px)':'100%','margin-left':configuration.ischannelName ? '88px':0,overflow: 'hidden',position: 'absolute',top:'22px'}"
        ...
        <div class="snap"
             :style="{left: snapLeft + 'px',top:'-36px'}"
             v-if="showSnap && showP">
          <span style="line-height: 90px;color:#000;"
                v-if="!spinShow&&!snapUrl">加载失败</span>
