...
export const formatRecordType = (type) => {
  if (type == 800 || type == 'timeVideo' || type == 1) {
    return '定时录像'
  } else if (type == 512) {
    return '手动录像'
...
export const openFlowData = async(data) => {
  // 本级预览开流
  ...
  let res
  try {
    res = await axios.post(`ctl/video`, obj)
  } catch (err) {
    return err
  }
...
export const openFlow = async(pluginDom, params) => {
  return pluginDom.OpenRealStreamEx(JSON.stringify(params))
}

/**
 * @description: 调用查询播放回放
 * @param {Object} data 插件播放所需数据
 * @param {string} type center 中心回放， frontEnd 中心回放
 * @param {Object} pluginDom 视频插件 dom
 * @return:
 * @author: dfk
 */
export const pluginPlayBackOpen = async(pluginDom, data, type = 'frontEnd') => {
  let param = {}
  if (type === 'center') {
    // {  data  数据
    //   startTime,
    ...
    let objOpen = {
      ...
      streamType: data.streamType === 'main' ? 1 : 2
    }
    let resOpen
    try {
      resOpen = await axios.post('/record/frontdevPlayBack', objOpen) // 前端回放录像查询
