      ...
      <div class="top">
        ...
        <div class="button-box">
          <el-button size="small"
                     @click="goBackList">返回列表</el-button>
        </div>
      </div>
      <el-tabs type="border-card"
        ...
        <el-tab-pane v-for="(tab, index) in tabs"
          ...
          <div class="tab-content"
            ...
            <relevance-comm ref="relevanceComm"
                            ...
                            @cancel="cancel"
                            @addLinks="addLinks"
                            @reduceLinks="reduceLinks"></relevance-comm>
          </div>
        </el-tab-pane>
      </el-tabs>
    </div>
...
import { getHostOrResourceApi } from '@src/http/parkingManage/parkingRecord.api.js'
...
export default {
  ...
  computed: {
    customizeQuery() {
      return {
        params: {
          type: 0,
          relOrgId: this.selectedData[0].orgId,
          subSystem: '1',
          channleType: 1,
        },
      }
  ...
  created() {
    ...
    getMoreLinks(ids).then((res) => {
      console.log(res, '联动配置')
      this.allReqLinks = res.data.data
      this.dealInitData(res.data.data)
    })
  },
  //     getHostOrResourceApi(this.selectedData[0].id,{ref:1}).then((res) => {
  //         console.log(res,'联动配置')
  //       this.allReqLinks = res.data.data
  //       this.dealInitData(res.data.data)
  //     })
  ...
  methods: {
    ...mapActions(['getHostOrResource', 'setHostOrResource']), // 获取指定主机的关联配置
      ...
      let gpData = [
        {
          ...
          type:
            this.configType === '停车配置' ? 'videoHost' : this.configType === '摄像机关联配置' ? 'vidicon' : 'alarm',
        },
      ]
      if (reqLink) {
