...
export default {
  ...
  computed: {
    wallName: function() {
      let result = this.wallList.filter(item => {
        // return item.value === this.linkData.wall
      })
  ...
  methods: {
    ...mapActions('equipmentConfigManagement', ['getAlarmlink', 'putAlarmlink']),
    // 编辑事件
    editClick() {
    ...
    setConfigData(resData) {
      ...
      this.linkData.receiveCheck = transBoolean(resData.dealAction) // 接警处置
      ...
      // this.linkData.wall = Number(resData.wall) // 联动上墙
    ...
    putAlarmlinkFuc() {
      ...
      this.saveData.name = this.alarmNameValue + '-' + this.alarmTypeValue 
      this.saveData.outputAction = transFT(this.linkData.centerCheck)
      this.saveData.dealAction = transFT(this.linkData.receiveCheck)
