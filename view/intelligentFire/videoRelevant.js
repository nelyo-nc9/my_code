    ...
    preview (row, column, event) {
      ...
      if (row.alarmTypeName === '设备报警') {
        ...
        if (row.partId && row.partId !== '0') {
          this.getGuardLinkFuc({ id: row.partId, ref: 2 }).then(res => {
            if (res) {
    ...
    previewPlay (item, detail) {
      ...
      item.map(val => {
        ...
        let data = {
          ...
          devIp: val.type === 4 ? val.ip : val.hostIp ? val.hostIp : val.ip,
          devCport: val.type === 4 ? val.cport : val.hostPort ? val.hostPort : val.cport,
          devDport: val.type === 4 ? val.dport : val.hostDport ? val.hostDport : val.dport,
          devPort: val.port || val.hostPort,
          streamType: val.streamType === 1 ? 'main' : (val.streamType === 2 ? 'sub1' : 'sub2'),
          // channel: 1,
          // devIp: '10.2.2.20',
          // devCport: 3721,
