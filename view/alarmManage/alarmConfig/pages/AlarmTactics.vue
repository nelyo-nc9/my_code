    ...
    <div class="alarm-tactics-content"
      ...
      <div class="alarm-tactics-main">
          ...
          <div class="tactics-main-right">
            <div v-for="(item, index) in colorList"
              ...
              <div style="width: 240px;height: 24px;margin-left:80px;-webkit-print-color-adjust: exact;border: 1px solid "
                   :style="{backgroundColor:item.color,borderColor:item.color}"></div>
            </div>
          </div>
        </div>
        <div class="tactics-main-row">
        ...
        <div class="tactics-main-row">
          <p>设备断网重复提醒间隔时间</p>
          <div class="tactics-main-right">
            <div style="width: 240px;height: 24px;border: 1px solid #ddddee;border-radius: 4px;padding: 0 10px;line-height: 24px">{{deviceremindTime}}</div>
          </div>
        </div>
        <div class="tactics-main-row tactics-back">
          <p>事件提醒提示类自动处理时间</p>
          ...
          <div class="tactics-main-right">
            <div v-for="(item, index) in videoList"
              ...
              <div class="voice-upload">
                ...
                <div style="width: 200px;height: 24px;border: 1px solid #ddddee;border-radius: 4px;padding: 0 10px;line-height: 24px;margin-left: 55px;">{{item.videoTime}}</div>
              </div>
              <div class="voice-upload">
                <span style="font-size: 12px">
                  <a v-if="item.checked"
                ...
                <div style="width: 200px;height: 24px;border: 1px solid #ddddee;border-radius: 4px;padding: 0 10px;line-height: 24px;margin-left: 55px;">{{item.preVideoTime}}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="tactics-main-right">
            ...
            <div class="add-row">
              <div v-for="(item, index) in armReason.reasonList"
                   ...
                   class="add-row-input">
                <i>{{item.name}}</i>
                <div style="width: 240px;height: 24px;border: 1px solid #ddddee;border-radius: 4px;padding: 0 10px;line-height: 24px;margin-left: 5px;display: inline-block">{{item.reason}}</div>
        ...
        <div class="tactics-main-row tactics-back">
          ...
          <div class="tactics-main-right">
            <span>
              <el-button size="mini"
                         @click="addDealClick"
                         :disabled="dealInfo.addBtn || editStatus">添加处置类型</el-button>
            </span>
            <div class="add-row">
              <div v-for="(item, index) in dealInfo.dealList"
                   :key="index"
                   class="add-row-input">
                <i>{{item.name}}</i>
                <div style="width: 210px;height: 24px;border: 1px solid #ddddee;border-radius: 4px;padding: 0 10px;line-height: 24px;margin-left: 5px;display: inline-block">{{item.dealTypeValue}}
                </div>
                <i>批量处理</i>
                <div style="width: 210px;height: 24px;border: 1px solid #ddddee;border-radius: 4px;padding: 0 10px;line-height: 24px;margin-left: 5px;display: inline-block">{{item.dealStatus}}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="alarm-tactics-content"
      ...
      <div class="alarm-tactics-main">
        ...
        <div class="tactics-main-row">
          <p>设备断网重复提醒间隔时间</p>
          <div class="tactics-main-right">
            <el-select v-model="deviceremindTime"
                       placeholder="请选择"
                       size="mini"
                       :disabled="editStatus">
              <el-option v-for="item in deviceTimeList"
                         :key="item.label"
                         :label="item.label"
                         :value="item.value"></el-option>
            </el-select>
            <!--            <el-input v-model="customizeTime" size="mini" v-if="promptTimeStatus" style="width: 200px" type="number"></el-input>-->
          </div>
        </div>
        <div class="tactics-main-row tactics-back">
          <p>事件提醒提示类自动处理时间</p>
          <div class="tactics-main-right">
            <span>
              <el-checkbox v-model="promptTimeCheck"
                           v-if="isStatus!==2"
                           :disabled="editStatus">启用</el-checkbox>
            </span>
            <span v-if="isStatus===2"> <input type="checkbox"
                     :checked="promptTimeCheck"
                     class="checkbox"
                     disabled>启用</span>
            <el-select v-model="promptTimeValue"
                       filterable
                       default-first-option
                       placeholder=""
                       @change="promptTimeSelect"
                       size="mini"
                       v-if="!promptTimeStatus"
                       :disabled="editStatus">
              <el-option v-for="item in promptTimeList"
                         :key="item.value"
                         :label="item.label"
                         :value="item.value"></el-option>
            </el-select>
            <el-input v-model="customizeTime"
                      size="mini"
                      :disabled="editStatus"
                      v-if="promptTimeStatus"
                      style="width: 200px"
        ...
        <div class="tactics-main-row">
          ...
          <div class="tactics-main-right">
            <span>
              <el-checkbox v-model="bypassReason.reasonCheck"
                           v-if="isStatus!==2"
                           :disabled="editStatus">启用</el-checkbox>
              <span v-if="isStatus===2"> <input type="checkbox"
...
export default {
  ...
  methods: {
    ...mapActions(['getPolicyConfiguration', 'putPolicyConfiguration']),
    ...
    // 添加预设原因
    addReasonClick() {
      let data = '预设原因' + (this.armReason.reasonList.length + 1)
    ...
    // 添加旁路原因
    addBypassClick() {
      let data = '预设原因' + (this.bypassReason.reasonList.length + 1)
    ...
    // 添加处置类型
    addDealClick() {
      let data = '处理类型' + (this.dealInfo.dealList.length + 1)
      this.dealInfo.dealList.push({
        name: data,
        dealTypeValue: '',
        dealStatus: 'yes'
      })
    ...
    setPolicyConfigurationData(resData) {
      ...
      this.colorList[2].color = resData.color3
      this.colorList[3].color = resData.color4
      this.colorList[4].color = resData.color5
      // 提示声音
      ...
      this.voiceList[2].autioUrl = resData.soundFile3
      this.voiceList[2].checked = transBoolean(resData.sound3)
      this.voiceList[3].autioUrl = resData.soundFile4
      this.voiceList[3].checked = transBoolean(resData.sound4)
      this.voiceList[4].autioUrl = resData.soundFile5
      this.voiceList[4].checked = transBoolean(resData.sound5)
      // 报警未处理升级时间
      ...
      this.upgradeList[2].upgradeTime = Number(resData.unproUpTime3)
      this.upgradeList[2].checked = transBoolean(resData.unproUp3)
      this.upgradeList[3].upgradeTime = Number(resData.unproUpTime4)
      this.upgradeList[3].checked = transBoolean(resData.unproUp4)
      this.upgradeList[4].upgradeTime = Number(resData.unproUpTime5)
      this.upgradeList[4].checked = transBoolean(resData.unproUp5)
      // 报警处理超时升级时间
      this.preupgradeList[0].preupgradeTime = Number(resData.proTimeoutUp1) || 0
      this.preupgradeList[0].checked = transBoolean(resData.proUpTime1) || false
      this.preupgradeList[1].preupgradeTime = Number(resData.proTimeoutUp2) || 0
      this.preupgradeList[1].checked = transBoolean(resData.proUpTime2) || false
      this.preupgradeList[2].preupgradeTime = Number(resData.proTimeoutUp3) || 0
      this.preupgradeList[2].checked = transBoolean(resData.proUpTime3) || false
      this.preupgradeList[3].preupgradeTime = Number(resData.proTimeoutUp4) || 0
      this.preupgradeList[3].checked = transBoolean(resData.proUpTime4) || false
      this.preupgradeList[4].preupgradeTime = Number(resData.proTimeoutUp5) || 0
      this.preupgradeList[4].checked = transBoolean(resData.proUpTime5) || false

      // 设备断网重复提醒间隔时间
      this.deviceremindTime = parseInt(resData.remindInterval)
      // 事件提醒提示类自动处理时间
      this.promptTimeCheck = transBoolean(resData.eventAutoPro)
      ...
      this.videoList[2].videoChecked = transBoolean(resData.record3)
      this.videoList[2].videoTime = Number(resData.recordTime3) || ''
      this.videoList[2].checked = transBoolean(resData.prerecord3)
      this.videoList[2].preVideoTime = Number(resData.prerecordTime3)
      this.videoList[3].videoChecked = transBoolean(resData.record4)
      this.videoList[3].videoTime = Number(resData.recordTime4) || ''
      this.videoList[3].checked = transBoolean(resData.prerecord4)
      this.videoList[3].preVideoTime = Number(resData.prerecordTime4)
      this.videoList[4].videoChecked = transBoolean(resData.record5)
      this.videoList[4].videoTime = Number(resData.recordTime5) || ''
      this.videoList[4].checked = transBoolean(resData.prerecord5)
      this.videoList[4].preVideoTime = Number(resData.prerecordTime5)
      // 远程布撤防
      this.armReason.reasonCheck = transBoolean(resData.armReasonOn)
      this.armReason.reasonList = JSON.parse(resData.armReason)
      this.dealInfo.dealList = JSON.parse(resData.processType)
      this.bypassReason.reasonList = (resData.bypassReason && JSON.parse(resData.bypassReason)) || []
    ...
    setPolicyConfigurationDataSave() {
      ...
      resData.color3 = this.colorList[2].color
      resData.color4 = this.colorList[3].color
      resData.color5 = this.colorList[4].color
      // 提示声音
      ...
      resData.soundFile3 = this.voiceList[2].autioUrl
      resData.sound3 = transFT(this.voiceList[2].checked)
      resData.soundFile4 = this.voiceList[3].autioUrl
      resData.sound4 = transFT(this.voiceList[3].checked)
      resData.soundFile5 = this.voiceList[4].autioUrl
      resData.sound5 = transFT(this.voiceList[4].checked)
      // 报警未处理升级时间
      ...
      resData.unproUpTime3 = Number(this.upgradeList[2].upgradeTime)
      resData.unproUp3 = transFT(this.upgradeList[2].checked)
      resData.unproUpTime4 = Number(this.upgradeList[3].upgradeTime)
      resData.unproUp4 = transFT(this.upgradeList[3].checked)
      resData.unproUpTime5 = Number(this.upgradeList[4].upgradeTime)
      resData.unproUp5 = transFT(this.upgradeList[4].checked)
      ...
      // this.preupgradeList[2].preupgradeTime = Number(resData.proTimeoutUp3)
      // this.preupgradeList[2].checked = Boolean(parseInt(resData.ProUpTime3))
      // this.preupgradeList[3].preupgradeTime = Number(resData.proTimeoutUp4)
      // this.preupgradeList[3].checked = Boolean(parseInt(resData.ProUpTime4))
      // this.preupgradeList[4].preupgradeTime = Number(resData.proTimeoutUp5)
      // this.preupgradeList[4].checked = Boolean(parseInt(resData.ProUpTime5))
      //
      resData.proTimeoutUp1 = Number(this.preupgradeList[0].preupgradeTime) || 0
      resData.proUpTime1 = transFT(this.preupgradeList[0].checked) || 0
      resData.proTimeoutUp2 = Number(this.preupgradeList[1].preupgradeTime) || 0
      resData.proUpTime2 = transFT(this.preupgradeList[1].checked) || 0
      resData.proTimeoutUp3 = Number(this.preupgradeList[2].preupgradeTime) || 0
      resData.proUpTime3 = transFT(this.preupgradeList[2].checked) || 0
      resData.proTimeoutUp4 = Number(this.preupgradeList[3].preupgradeTime) || 0
      resData.proUpTime4 = transFT(this.preupgradeList[3].checked) || 0
      resData.proTimeoutUp5 = Number(this.preupgradeList[4].preupgradeTime) || 0
      resData.proUpTime5 = transFT(this.preupgradeList[4].checked) || 0

      // // 设备断网重复提醒间隔时间
      resData.remindInterval = Number(this.deviceremindTime)
      // 事件提醒提示类自动处理时间
      resData.eventAutoPro = transFT(this.promptTimeCheck)
      ...
      resData.record3 = transFT(this.videoList[2].videoChecked)
      resData.recordTime3 = Number(this.videoList[2].videoTime)
      resData.prerecord3 = transFT(this.videoList[2].checked)
      resData.prerecordTime3 = Number(this.videoList[2].preVideoTime)
      resData.record4 = transFT(this.videoList[3].videoChecked)
      resData.recordTime4 = Number(this.videoList[3].videoTime)
      resData.prerecord4 = transFT(this.videoList[3].checked)
      resData.prerecordTime4 = Number(this.videoList[3].preVideoTime)
      resData.record5 = transFT(this.videoList[4].videoChecked)
      resData.recordTime5 = Number(this.videoList[4].videoTime)
      resData.prerecord5 = transFT(this.videoList[4].checked)
      resData.prerecordTime5 = Number(this.videoList[4].preVideoTime)
      // // 远程布撤防
      resData.armReasonOn = transFT(this.armReason.reasonCheck)
      resData.armReason = JSON.stringify(this.armReason.reasonList)
      resData.processType = JSON.stringify(this.dealInfo.dealList)
      resData.bypassReason = JSON.stringify(this.bypassReason.reasonList)
