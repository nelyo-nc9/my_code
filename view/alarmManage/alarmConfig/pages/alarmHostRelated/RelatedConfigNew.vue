...
<script>
import { getHostMoreLinks, setHostMoreLinks } from '@src/http/alarm/alarmHostConfig.api.js'
...
export default {
  ...
  created() {
    ...
    this.selectedData.forEach(item => {
      ids.push({
        id: item.id,
        ref: this.configType === '报警主机关联配置' ? 1 : 2
      })
    })
    // 根据列表页选中的设备或者通道拿到id 去获取关联配置页面的目标列的数据
    getHostMoreLinks(ids).then(res => {
  ...
  methods: {
    ...
    dealDisableData(reqData) {
      ...
      reqData.children &&
        reqData.children.forEach(item => {
          disabledData.push('res' + (item.rcId || item.id))
        })
    ...
    save() {
      ...
      this.selectedData.forEach(i => {
        let obj = {
          id: i.id,
          type: this.configType === '报警主机关联配置' ? 1 : 2
        }
        let linkages = {}
      ...
      setHostMoreLinks(sendData).then(res => {
        if (res.data.code === 0) {
    ...
    handleNodeClick(data, node) {
      // 目标树点击事件
      console.log(data.rcId, data.tierType, '============')
      ...
      let gpData = [
        {
          ...
          type:
            this.configType === '报警主机关联配置'
              ? 'videoHost'
              : this.configType === '探测器关联配置'
              ? 'videoHost'
              : 'alarm'
        }
      ]
