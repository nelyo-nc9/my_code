<template>
  <div class="container">
    ...
    <div class="right-contain">
      ...
      <div class="right-btn-box">
        <div class="btn-group">
          ...
          <el-button size="mini"
                     @click="focusResourcesdialogVisibleFuc">重点关注资源</el-button>
          <el-button size="mini"
                     :disabled="selectionTableData.length===0"
                     @click="alarmPushDialogVisible=true">报警推送</el-button>
          <el-button size="mini"
                     :type='this.Statistics.focus>0?"danger":"default"'
                     @click="getAlarmReceiveListFuc('focus')">重点资源报警：{{Statistics.focus}}</el-button>
          <el-button size="mini"
              ...
              <el-form ref="form"
                ...
                <el-form-item>
                  <el-checkbox-group v-model="filterForm.type">
                    <el-checkbox label="入侵报警"
                                 name="type"></el-checkbox>
                    <el-checkbox label="门禁报警"
                                 name="type"></el-checkbox>
                    <el-checkbox label="对讲报警"
                                 name="type"></el-checkbox>
                    <el-checkbox label="视频报警"
                                 name="type"></el-checkbox>
                    <el-checkbox label="智能报警"
                                 name="type"></el-checkbox>
                    <el-checkbox label="出入口报警"
                                 name="type"></el-checkbox>
                    <el-checkbox label="防护舱报警"
                                 name="type"></el-checkbox>
                    <el-checkbox label="停车场报警"
                                 name="type"></el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
                <el-form-item>
                  <el-button type="primary"
                             @click="filteronSubmit"
                             size="mini">确认</el-button>
                  <el-button size="mini"
                             @click="filterVisible = false">取消</el-button>
                </el-form-item>
              </el-form>
            </div>
    ...
    <!--        删除报警-->
    <div>
      <el-dialog title="移除报警"
                 :visible.sync="deletedialogVisible"
                 width="20%"
                 center
                 :before-close="deleteHandleClose">
        <span>请确认移除所选报警信息吗？</span>
        <span slot="footer"
              class="dialog-footer">
...
export default {
    ...
    return {
      ...
      headerArray: [
        ...
        },
        {
          ...
          count: 0,
          clickFuc: () => {
            return this.getAlarmReceiveListFuc(null, 3)
          },
          level: 3
        },
        {
          ...
          count: 0,
          clickFuc: () => {
            return this.getAlarmReceiveListFuc(null, 4)
          },
          level: 4
        },
        {
          ...
          count: 0,
          clickFuc: () => {
            return this.getAlarmReceiveListFuc(null, 5)
          },
          level: 5
        },
        {
      ...
      actHeaderBtn: null,
      tableDataRealtime: [], // 直接说实时报警
      tableDataRealtimePlice: [],
      tableData: [],
      ...
      loading: false,
      filterForm: {
        type: ['入侵报警', '门禁报警', '对讲报警', '视频报警', '智能报警', '出入口报警', '停车场报警', '防护舱报警']
      },
      dealSubmit: {
        // 处置接口
        dealType: null,
  ...
  computed: {
    ...mapState('alarmReceive', {
      alarmDealTableData: 'alarmDealTableData'
    }),
    ...mapGetters('alarmReceive', ['stopResiveStatas'])
  },
  created() {
  ...
  mounted() {
    ...
    this.scrollDomSetTimeout = setTimeout(() => {
      ...
      scrollDom.addEventListener('scroll', () => {
        ...
        if (scrollDistance <= 0) {
          ...
          if (this.scrollParam.pageNum > 1) {
            // this.$message({
            //   type: 'error',
            //   message: '请优先处理最新报警，处理完毕后可继续加载！'
            // })
            return
          }
      ...
      // 页面接收数据放在统计数据回来之后，因为现在有实时和历史所以暂时延迟时间处理，后期优化
      this.addEventSocketFuc()
    }, 3000)
  },
  methods: {
    ...mapMutations('alarmReceive', {
    ...
    ...mapActions(['getPolicyConfiguration', 'getAlarmlink']),
        ...
        for (let i = 0; i < this.ids.length; i++) {
          ...
          if (newTime - item.currentTime > timeInterval) {
            for (let j = 0; j < this.tableDataRealtime.length; j++) {
              ...
              if (val.isMerge && val.ip + val.channel + val.eventTypeName === item.id) {
    ...
    // 判断是不是要弹出视频
    isVideoFuc(data) {
      if (this.policyConfiguration && data.level && this.policyConfiguration[data.level - 1].videoCheckPopup === 'T') {
        return true
      } else {
        return false
      }
    ...
    isVideoCheckFuc(data) {
      if (this.policyConfiguration.length > 0 && this.policyConfiguration[data.level - 1].videoCheck === 'T') {
        return true
      } else {
        return false
      }
    ...
    // 判断是不是要不要到接警处置页面
    isDealCheckFuc(data) {
      if (this.policyConfiguration.length > 0 && this.policyConfiguration[data.level - 1].alarmProAction === 'T') {
        return true
      } else {
        return false
      }
    ...
    getAlarmlinkFuc() {
      let that = this
      Promise.all([
        this.getAlarmlink({ level: 1 }),
        this.getAlarmlink({ level: 2 }),
        this.getAlarmlink({ level: 3 }),
        this.getAlarmlink({ level: 4 }),
        this.getAlarmlink({ level: 5 })
      ]).then(res => {
        if (res) {
          res.map(item => {
            if (item.data && item.data && item.data.results && item.data.results.length > 0) {
    ...
    socketFuc(data) {
      ...
      if (this.actHeaderBtn && this.actHeaderBtn === parseInt(data.alarmInfo.level)) {
        this.homologousMerging(
          data.alarmInfo.ip + data.alarmInfo.channel + data.alarmInfo.eventTypeName,
          this.ids,
          data.alarmInfo
        )
      } else if (!this.actHeaderBtn) {
        // 根据报警等级配置看是否要显示，不是根据12等级
    ...
    findCountFuc(level) {
      ...
      this.findCount(param).then(res => {
        ...
        this.Statistics.total = this.countParam.alarmInfoTotal
        ...
        this.headerArray[3].totalCount = this.countParam.level3
        this.headerArray[3].count = this.countParam.level3Done
    ...
    homologousMerging(id, ids, data) {
      ...
      if (length) {
        this.tableDataRealtime.map(item => {
          let str = item.ip + item.channel + item.eventTypeName
          if (item.isMerge && str === id) {
            let count = item.mergeNo
            // _.assign(item, _.pickBy(data, _.identity))   因为有双向选中，所以后来的报警不能覆盖之前的
        ...
        this.ids.push({
          id: data.ip + data.channel + data.eventTypeName,
        ...
        // 实时界面只默认显示最多50条
        if (this.tableDataRealtime.length > 50) {
        ...
        this.tableDataRealtimePlice = _.assign([], this.tableDataRealtime)
        console.log(this.tableDataRealtime)
      }
    },
    bottomMouseDown() {
    ...
    getPolicyConfigurationFuc() {
      this.getPolicyConfiguration().then(res => {
        if (res) {
          ...
          this.audioPlayUrlArray = [data.soundFile1, data.soundFile2, data.soundFile3, data.soundFile4, data.soundFile5]
          this.audioPlayUrlArrayCheck = [data.sound1, data.sound2, data.sound3, data.sound4, data.sound5]
        }
      })
    },
    // 批量报警处置，报警处置提交
        ...
        if (this.selectionTableData.length > 0) {
          for (let j = 0; j < this.selectionTableData.length; j++) {
            for (let i = 0; i < this.ids.length; i++) {
              if (
                this.ids[i].id ===
                this.selectionTableData[j].ip +
                  this.selectionTableData[j].channel +
          ...
          let index = this.ids.findIndex(item => {
            return item.id === this.rowClickParam.ip + this.rowClickParam.channel + this.rowClickParam.eventTypeName
          })
    ...
    deleteHandleFuc() {
      ...
      this.alarmDealRemove(param).then(res => {
        if (res) {
          this.postAlarmdisposeendFuc(ids) // 处理完后给es 通知
          if (this.selectionTableData.length > 0) {
            for (let j = 0; j < this.selectionTableData.length; j++) {
              for (let i = 0; i < this.ids.length; i++) {
                if (
      ...
      this.getSmsIp(param).then(res => {
        if (res) {
          if (res.data.isPush === '1') {
            this.SmsIp = res.data.httpUrl
            this.shortMessage = res.data
          }
        }
      })
    },
    // 发送短信
    sendMsgFuc() {
      if (!this.shortMessage || (this.shortMessage && this.shortMessage.isPush === '2')) {
        this.$message({
          type: 'error',
          message: '短信功能未开通！'
        })
    ...
    // 过滤确认按钮
    filteronSubmit() {
      if (this.acceptancebtnsActive === 2) {
        ...
        this.tableData = data
      } else if (this.acceptancebtnsActive === 1) {
        //  tableDataRealtimePlice
        if (this.filterForm.type.length === 0) {
          this.tableDataRealtime = this.tableDataRealtimePlice
          return
        }
        let data = _.assign([], this.tableDataRealtimePlice)
        data.map((item, i) => {
          if (this.filterForm.type.indexOf(item.alarmTypeName) === -1) {
            delete data[i]
          }
        })
        this.tableDataRealtime = data
      }
    ...
    rowClick(row) {
      ...
      if (row) {
        ...
        this.$refs.preview && this.$refs.preview.syncChangeCheckedPane(row)
        ...
        if (row.alarmTypeName === '门禁报警') {
          ...
          if (row.resourceId) {
            this.findVideoLinkFuc({ id: row.resourceId, ref: 2 })
          } else {
            this.findVideoLinkFuc({ id: row.deviceId, ref: 1 })
          }
        }
      }
      this.linkData = []
  ...
  destroyed() {
    console.log('销毁')
    let scrollDom = document.getElementsByClassName('el-table__body-wrapper')[0]
    scrollDom.removeEventListener()
    clearTimeout(this.scrollDomSetTimeout)
