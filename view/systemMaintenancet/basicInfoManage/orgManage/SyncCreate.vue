<template>
  <div class="org-manage-sync-create-box">
    <!-- 机构管理-机构维护-新增机构列表 -->
    <div class="content">
      <!-- 顶部操作栏 -->
      <div class="top">
        ...
        <div class="button-box">
          <el-button size="small" :disabled="disabledAll" @click="allSync" v-if="isBtn('全部同步')">全部同步</el-button>
          <el-button size="small" :disabled="disabledAll" @click="allNoSync" v-if="isBtn('全部不同步')">全部不同步</el-button>
          <el-button size="small" :disabled="disabledSync" @click="clickSync" v-if="isBtn('同步')">同步</el-button>
          <el-button size="small" :disabled="disabledSync" @click="clickNoSync" v-if="isBtn('不同步')">不同步</el-button>
        </div>
      </div>
      <div class="main">
        <!-- 表格和分页 -->
...
export default {
  ...
  data() {
    return {
      resizable: true,
      columns: [
        { prop: 'updatedAt', sortable: true, label: '新增时间', width: '150' },
      ...
      tableData: [],
      selectData: [],
      disabledSync: true, // 同步和不同步按钮是否禁用
      disabledAll: true, // 全部按钮是否禁用
      total: 0,
      page: 1,
      limit: 100,
      name: '' // 查询的机构名称
  ...
  methods: {
    ...
    getTableList() { // 请求表格数据方法
      let param = {
        ...
        limit: this.limit,
        name: this.name,
        type: 1 // 1-新增机构，2-撤销机构，3-合并机构，4-迁移机构
      }
    ...
    allSync() {
      this.$refs.createTable.toggleAllSelection()
      setTimeout(() => {
        this.setOrgSync(6)
      }, 100)
    },
    ...
    setOrgSync(val) { // 调用机构同步接口
      ...
      let sName = val === 1 || val === 6 ?'机构同步' : '取消同步'
