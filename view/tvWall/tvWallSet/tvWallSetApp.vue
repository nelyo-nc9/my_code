<template>
  <div class="tvWallSetWrap">
    <div class="setWrapTop">
      <ul class="ulStyle">
        <li class="liStyle"
            @click="ActionMethod(0)">
          <span>删除</span>
        </li>
        <li class="liStyle"
            @click="ActionMethod(1)"
            v-if="tvWallMessage.type === 2">
          <span>开窗</span>
        </li>
        <li class="liStyle"
            @click="ActionMethod(2)"
            v-if="tvWallMessage.type === 2">
          <span>拼接</span>
        </li>
        <li class="liStyle"
            @click="ActionMethod(3)"
            v-if="tvWallMessage.type === 2">
          <span>拆分</span>
        </li>
        <li class="liStyle">
        ...
        <li class="liStyle"
            v-if="!showNumber1"
            @click="ActionMethod(5, '显示编号')">
          <span>显示编号</span>
        </li>
        <li class="liStyle"
            v-if="showNumber1"
            @click="ActionMethod(5, '隐藏编号')">
          <span>隐藏编号</span>
        </li>
        <li class="liStyle"
            @click="ActionMethod(6)">
          <span>设置为报警屏</span>
        </li>
      </ul>
      <div class="resouse"
      ...
      <div class="refersh"
        ...
        <el-button type="text"
                   ...
                   @click="refreshTvWall">刷新电视墙</el-button>
      </div>
    </div>
    <div class="setWrapContend"
      ...
      <!-- 创建电视墙底板弹框 -->
      <el-dialog title="创建电视墙"
                 :visible.sync="dialogVisible"
                 width="30%">
        <ul class="canvansCount">
          <li>
            <div class="const">
              <span>横向</span>
            </div>
        ...
        <span slot="footer"
          ...
          <el-button size="small"
                     type="primary"
                     @click="canvasMethod">确 定</el-button>
        </span>
      </el-dialog>
    </div>
    <!-- 双击已经上流的窗口，弹出弹框，播放视频 -->
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      channlAndWindow: ({ tvWall }) => tvWall.channlAndWindow // 接收窗口和通道绑定后传过来的设备id和状态，分别用于资源上墙和计算窗口数量
    })
  },
  watch: {
    ...
    channlAndWindow: {
      ...
      handler(newVal, oldVal) {
        this.getWindowCount_fuc() // 当通道和窗口绑定后，重新计算解码窗口总数和使用数以及未使用数
      },
      deep: true
    }
  },
  methods: {
        ...
        for (let i = 0; i < tvWallData.rtlayout.row; i++) {
          ...
          for (let j = 0; j < tvWallData.rtlayout.column; j++) {
            let tdObj = {
              ...
              devCtl: [{ rid: 1, paneCount: 1, left: 0, right: 0, top: 0, bottom: 0 }], // 资源上墙参数
              ...
              associatedChannelName: '', // 关联通道名称
              associateddeviceId: '', // 关联设备ID
              showNumber: 0, // 显示编号
     ...
     * 是否允许绘制底板
     * 未绑定电视墙属性，不允许绘制底板
     * 需到电视墙管理中绑定电视墙属于那种解码类型
     * type:1(解码器)
     * type:2(拼控解码一体机)
     */
    Tv_fool() {
      if (this.tvWallMessage.type) {
        this.dialogVisible = true
      } else {
        this.failureMethod('请绑定电视墙属性')
      }
    },
    //   点击绘制地板
    canvasMethod() {
      this.column = parseInt(this.inputTwo)
      this.row = parseInt(this.input)
      if (this.column && this.row) {
        ...
        this.createdTvWallFloor(params)
          .then(res => {
            if (res.data.code === 0) {
              this.getWindowCount_fuc() // 模拟电视墙创建成功，重新计算窗口总数，已使用和未使用数
              this.input = '' // 创建成功，清除输入框
              this.inputTwo = '' // 创建成功，清空输入框
              this.showOrclose = false
              ...
              this.SET_CREATED_FLOOR(sendObj) // 底板创建成功，发送信号到index页面重新拉取列表数据
            } else {
        ...
        this.showOrclose = true
        this.dialogVisible = true
        this.warningMethod('请输入正确的行数和列数')
      }
    },
    // 点击头部按钮，所作出对应操作
    ActionMethod(index, val) {
      switch (index) {
        case 0:
          this.delectMethod('此操作将删除模拟电视墙, 是否继续?', 1)
          break
        case 1:
          this.openWindow()
          break
          ...
          .then(() => {
            if (num === 1) {
              ...
              this.deleteTvWallFloor(params)
                .then(res => {
                  if (res.data.code === 0) {
                    this.getWindowCount_fuc() // 删除地板成功，重新计算窗口总数，已使用和未使用数
              ...
              this.refreshTvWallApi(params)
                .then(res => {
                  if (res.data.code === 0) {
                    this.succefullMethod('电视墙刷新成功')
                    // 刷新成功后向电视墙列表组件发送信号让电视墙列表组件重新拉取电视墙列表
                    let sendObj = {
                      state: true,
                      id: this.tvWallMessage.id
                    }
    ...
    // 点击任意资源已上墙窗口，获取播放信息请求
    getWindowVideo() {
      this.getcurstatus(this.WindowVideoParams)
        .then(res => {
          if (res.data.code === 0) {
            this.SET_VIDEO_PARAMS(res.data.data) // 将播放需要的信息发到vuex中
          } else {
            let params = {}
      ...
      if (this.tvWallMessage.rtlayout.row && this.tvWallMessage.rtlayout.column) {
        if (this.macAderss !== '' && windowCodeArr.length !== 0 && index === 1) {
          ...
          if (codeArr.length === 0) {
            ...
            // if (secondJoin) { // 二次拼接激活，不允许二次拼接
            //   this.failureMethod('暂不允许二次拼接')
            //   return
            // }
    ...
    sendWindowIndex(val, checkedState, td) {
      ...
      if (this.windowmergewnd.beforewnd.length === 0) {
        ...
        // if (td.isJoin === false) {
        //   td.windowsRect[0]['netIndex'] = td.netIndex
        //   d = this.$lodash.cloneDeep(this.windowmergewnd.beforewnd)
        //   d = d.filter(item => td.windowsRect.some(i => i.code === item.code))
        // } else if (td.isJoin === true) {
        //   td.jointCoordinates[0]['netIndex'] = td.netIndex
        //   td.jointCoordinates[0]['code'] = td.windowIndex
        //   j = this.$lodash.cloneDeep(this.windowmergewnd.beforewnd)
        //   j = j.filter(item => td.jointCoordinates.some(i => i.code === item.code))
        // }
      }
      let sortcode = (a, b) => {
        // 排序
    ...
    // 刷新电视墙
    refreshTvWall() {
      this.delectMethod('此操作将刷新电视墙, 是否继续?', 4)
    },
    // 接受socket
    connectTvWall() {
      // 接收socket
     ...
     * 3.解码器 + 拼控:  type = 4 nomal3_function
     * */
          ...
          for (let j = 0; j < item.length; j++) {
            ...
            emit_data.map(item => {
              if (item.monitor === val.windowIndex) {
                ...
                val.associateddeviceId = item.deviceId // 通道绑定(设备ID)
                console.log(val)
                console.log(item)
