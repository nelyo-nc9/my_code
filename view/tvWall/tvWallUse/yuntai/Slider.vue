...
<template>
  <div :class="[sliderClass, disabled ? sliderClass + '-disabled': '']" @mouseover="mouse(true)" @mouseleave="mouse(false)">
    ...
    <input :class="[sliderClass + '-input']" type="text" style="outline:none;height: 18px;background:#fff;color:#656565;border:1px solid #E1E1E1;" v-show="range && showInput" v-model='sliderValue' :disabled="disabled" @input="input" @change="$emit('change',sliderValue)" />
  </div>
</template>
<script>
