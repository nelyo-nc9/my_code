      ...
      <div class="top-box">
        ...
        <div class="button-box">
          <el-button size="small"
                     icon="el-icon-plus"
                     @click="skipTo('AddDecoding', 1)">新建</el-button>
          <el-button :disabled="multipleSelection.length !== 1"
                     size="small"
                     icon="el-icon-edit-outline"
                     @click="skipTo('ChangeDecod', 2)">编辑</el-button>
          ...
          <el-button :disabled="multipleSelection.length !== 1"
                     size="small"
                     icon="el-icon-edit-outline"
                     @click="gainData">获取</el-button>
          <el-button :disabled="multipleSelection.length === 0"
                     size="small"
                     icon="icon iconfont-ccb ccbqianyi el-icon-"
                     @click="skipTo('moveDecod', 3)">迁移</el-button>
          <el-button :disabled="multipleSelection.length !== 1"
                     size="small"
                     icon="el-icon-sort"
                     @click="skipTo('SortDecoding', 4)">排序</el-button>
      ...
      <div class="table-box">
        <div class="table-top">
          ...
          <el-input v-model="input"
                    ...
                    size="mini"></el-input>
          <el-select style="float:right;width:85px;margin-right:12px;"
                     v-model="status"
                     size="mini"
                     @change="search_State">
            <el-option label="全部"
...
export default {
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        // { prop: 'password', label: '密码', width: '', sortable: true },
        { prop: 'brandName', label: '品牌', width: '', sortable: true },
        { prop: 'modelNum', label: '型号', width: '', sortable: true },
        { prop: 'typeName', label: '设备类型', width: '', sortable: true },
        { prop: 'accessProtocol', label: '接入方式', width: '', sortable: true },
        { prop: 'decoderNumber', label: '解码窗口数量', width: '130', sortable: true },
        { prop: 'inputNumber', label: '本地终端输入数量', width: '150', sortable: true },
        { prop: 'outputNumber', label: '输出通道数量', width: '130', sortable: true },
        { prop: 'statusName', label: '在线状态', width: '', sortable: true },
        { prop: 'assetCode', label: '资产编码', width: '', sortable: true },
        { prop: 'serialCode', label: '序列号', width: '', sortable: true },
  ...
  created() { },
  mounted() {
    this.getDecodingDeviceList(1) // 页面加载完成，获取解码设备列表
  ...
  methods: {
    ...
    ...mapActions(['getDecodingDevice', 'deleteDecodingDevice', 'obtainApi']),
    getTreeApi(params) {
        ...
        .then(res => {
          if (res.data.code === 0) {
            ...
            this.tableData.map(item => {
              ...
              item['typeName'] =
                item.type === 1 ? '解码器' : item.type === 2 ? '解码拼控一体机' : item.type === 3 ? 'LED设备' : item.type === 4 ? '解码器' : ''
              item['brandName'] =
                item.brand === 'Hikvision' ? '海康' : item.brand === 'Bstar' ? '蓝色星际' : item.brand === 'DaHua' ? '大华' : ''
            })
    ...
    deleteMethod() {
      if (this.multipleSelection.length === 0) {
        ...
        this.deviceModel('该解码设备')
      }
    },
    // 获取设备真实信息(本地终端输入数量和输出数量)
    gainData() {
      let params = {
        id: this.multipleSelection[0].id
      }
      this.obtainApi(params).then(res => {
        if (res.data.code === 0) {
          this.succefullMethod('设备信息获取成功')
          this.getDecodingDeviceList(1) // 刷新列表
        }
      }).catch(err => {
        console.log(err)
    ...
    exportMethod() {
      ...
      export_json_to_excel(tHeader, data, '解码设备配置列表')
    },
    ...
    Toprint() {
      ...
      printJS({
        ...
        properties: [
          ...
          { field: 'typeName', displayName: '设备类型' },
          { field: 'accessProtocol', displayName: '接入方式' },
          { field: 'decoderNumber', displayName: '解码窗口数量' },
          { field: 'inputNumber', displayName: '本地终端输入数量' },
          { field: 'outputNumber', displayName: '输出通道数量' },
          { field: 'statusName', displayName: '在线状态' },
          { field: 'assetCode', displayName: '资产编码' },
          { field: 'serialCode', displayName: '序列号' },
        ...
        type: 'json',
        // 表题
        header: '<p class="custom">解码设备配置列表</p>',
        // 表题样式
