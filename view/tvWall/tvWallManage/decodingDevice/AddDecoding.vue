<template>
  <div class="content">
    ...
    <div class="right-content">
      ...
      <div class="form-box">
        <div class="form-title">解码设备信息</div>
          ...
          <div class="left-form">
            <el-form ref="addForm"
              ...
              <el-form-item label="解码窗口数量"
                            class="maBottom"
                            prop="decoderNumber">
                <el-input v-model="formData.decoderNumber"></el-input>
              </el-form-item>
          ...
          <div class="right-form">
            <el-form size="mini"
              ...
              <el-form-item label="设备序列号"
                            class="maBottom">
                <el-input disabled
                          v-model="formData.serialCode"></el-input>
              </el-form-item>
              ...
              <el-form-item label="本地终端输入数量"
                            class="maBottom"
                            prop="inputNumber">
                <el-input :disabled="localFlag"
                          placeholder="解码器无本地终端输入"
                          v-model="formData.inputNumber"></el-input>
              </el-form-item>
            </el-form>
          </div>
        </div>
        ...
        <div class="form-footer">
          <el-button type="primary"
                     size="small"
                     @click="sucefullSave">保存</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      localFlag: true, // 本地终端输入数量input的开关(解码器无本地终端输入)
  ...
  methods: {
    ...
    ...mapActions(['addDecodingDevice', 'getTvWallMenuList', 'chanegeTvWallApi', 'getservice', 'getdictionariesdata']),
      ...
      this.$refs['addForm'].validate(valid => {
        this.$refs['addForm2'].validate(valid2 => {
          if (valid && valid2) {
            ...
            // delete this.formData.typeName
            // delete this.formData.wallName
            if (this.formData.typeName === '解码器' && this.formData.brandName === '海康威视') {
              // 第三种解码器:M3+B20
              this.formData.type = 4
            } else if (this.formData.typeName === '解码器') {
              this.formData.type = 1
            }
    ...
    getcopy_data() {
      if (this.formData_copy.orgId) {
        ...
        this.formData.serialCode = this.formData_copy.serialCode
        this.formData.decoderNumber = this.formData_copy.decoderNumber
        ...
        this.formData.typeName = this.formData_copy.typeName
        this.formData.port = this.formData_copy.port
        this.formData.password = this.formData_copy.password
        this.formData.inputNumber = this.formData_copy.inputNumber
        this.formData.outputNumber = this.formData_copy.outputNumber
      }
    },
    // 成功提示
    succefullMethod(val) {
    ...
    Tvdictionaries() {
      ...
      this.getTvWallMenuList()
        .then(res => {
          console.log('电视墙初始化字典数据 ----------------- ', res.data.data.results)
