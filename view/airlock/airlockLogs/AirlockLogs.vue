        ...
        <el-tabs v-model="activeName"
          ...
          <el-tab-pane label="操作日志"
            ...
            <div class="from-button"
              ...
              <el-button size="small"
                         :disabled="operationTableData.length===0"
                         @click="operationGetTabList">导出</el-button>
            </div>
            <el-table :data="operationTableData"
                      style="width: 100%"
                      :height="operationShowFrom?'430px':'686px'"
          ...
          <el-tab-pane label="事件日志"
            ...
            <div class="query-form"
              ...
              <el-form ref="eventForm"
                ...
                <el-form-item label="所属门禁平台"
                              prop="guardPlatformName">
                  <el-input v-model="eventForm.guardPlatformName"
                            placeholder="请输入门禁平台名称"></el-input>
                </el-form-item>
                <el-form-item label="所属门禁主机"
                              prop="guardHostName">
                  <el-input v-model="eventForm.guardDeviceName"
                            placeholder="请输入门禁主机名称"></el-input>
                </el-form-item>
                <el-form-item label="门禁通道名称"
                              prop='guardChannelName'>
                  <el-input v-model="eventForm.guardChannelName"
                            placeholder="请输入门禁通道名称"></el-input>
                </el-form-item>
                <el-form-item label="日志类型"
                              prop="eventName">
                  <el-input v-model="eventForm.eventName"
                            placeholder="请输入日志类型"></el-input>
                </el-form-item>
                <el-form-item label="卡号"
                              prop='cardNumber'>
                  <el-input v-model="eventForm.cardNumber"
                ...
                <el-form-item label="性别">
                  <el-select v-model="eventForm.sex"
                             placeholder="请选择性别">
                    <el-option label="全部"
                               value="">
                    </el-option>
                ...
                <el-form-item label="身份证"
                              prop='identityCard'>
                  <el-input v-model="eventForm.identityCard"
                            placeholder="请输入身份证号"></el-input>
                </el-form-item>
                <el-form-item label="部门"
                              prop='department'>
                  <el-input v-model="eventForm.department"
                            placeholder="请输入部门"></el-input>
                </el-form-item>
                <el-form-item label="职务"
                              prop='job'>
                  <el-input v-model="eventForm.job"
            ...
            <el-table :data="eventTableData"
                      style="width: 100%"
                      :height="eventShowFrom?'368px':'686px'"
                      :default-sort="{ prop: 'date', order: 'descending' }"
                      align="left"
                      tooltip-effect="dark"
...
import {
  ...
  getPoliceList,
  getEvent,
} from '../../../http/InterlockingDoor/linkedAirlockLog.api.js'
import { getSearchOrgApi } from '@src/http/video/recordConfig.api.js'
import { download } from '@src/common/download.js'
import rules from '@src/assets/rules/validation/security/airlock.js'
export default {
  components: {
    VideoPagination,
  },
  data() {
    return {
      ...
      eventPropData: [
        ...
        { prop: 'guardDeviceName', label: '所属门禁主机', width: '', sortable: 'true' },
        { prop: 'guardChannelName', label: '互锁门通道名称', width: '', sortable: 'true' },
        { prop: 'eventName', label: '日志类型', width: '', sortable: 'true' },
        { prop: 'occurTime', label: '时间', width: '', sortable: 'true' },
        { prop: 'cardNumber', label: '卡号', width: '', sortable: 'true' },
        { prop: 'name', label: '姓名', width: '', sortable: 'true' },
        { prop: 'sex', label: '性别', width: '', sortable: 'true' },
        { prop: 'identityCard', label: '身份证', width: '', sortable: 'true' },
        { prop: 'department', label: '部门', width: '', sortable: 'true' },
        { prop: 'job', label: '职位', width: '', sortable: 'true' },
      ],
      eventTotal: 0,
      operationForm: {
        page: '1',
      ...
      operationPropData: [
        ...
        { prop: 'operateType', label: '操作类型', width: '', sortable: 'true' },
  ...
  methods: {
        ...
        this.$refs['eventForm'].validate((valid) => {
          if (valid) {
            ...
            eventLog(body).then((res) => {
              if (res.data.data && res.data.data.doors) {
                res.data.data.doors.forEach((element, index) => {
                  element.index = index + 1
                })
                this.eventTableData = res.data.data.doors
    ...
    eventGetTabList() {
      ...
      if (this.eventMultipleSelection && this.eventMultipleSelection.length > 0) {
        ...
        export_json_to_excel(
          tHeader,
          data,
          `互锁门事件日志导出${this.$moment(new Date()).format('YYYY-MM-DD HH-mm-ss')}`
        )
        ...
        // zp去掉url中间的/host
        let url = `${window.location.protocol}//${window.location.hostname}:${window.location.port}/api/guard/v1/door/record/explode`
        let name = `互锁门事件日志导出${this.$moment(new Date()).format('YYYY-MM-DD HH-mm-ss')}.xlsx`
        download(
          url,
          name,
          () => {},
    ...
    operationGetTabList() {
      ...
      if (this.operationMultipleSelection && this.operationMultipleSelection.length > 0) {
        let list = this.operationMultipleSelection
        const data = this.formatJson(filterVal, list)
        export_json_to_excel(
          tHeader,
          data,
          `互锁门操作日志导出${this.$moment(new Date()).format('YYYY-MM-DD HH-mm-ss')}`
        )
        ...
        // zp去掉url中间的/host
        let url = `${window.location.protocol}//${window.location.hostname}:${window.location.port}/api/guard/v1/operatelog/door/explode`
        let name = `互锁门操作日志导出${this.$moment(new Date()).format('YYYY-MM-DD HH-mm-ss')}.xlsx`
        download(
          url,
          name,
          () => {},
