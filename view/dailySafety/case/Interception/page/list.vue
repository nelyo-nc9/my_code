<template>
  <div class="main">
    ...
    <div style="margin:20px 0;" v-show="isShowInfo==false">
      ...
      <el-button style="font-size:12px;" plain @click="exportFile('成功堵截案例管理')">导出</el-button>
      <el-button style="font-size:12px;" plain @click="Toprint('成功堵截案例管理')">打印</el-button>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      tableColumn: [
        // 表格标题行内容
        { prop: 'orgName', label: '堵截案例名称', width: '', sortable: 'true' },
  ...
  methods: {
    ...
    Toprint(title) {
      ...
      printJS({
        ...
        properties: [
          { field: 'orgName', displayName: '堵截案例名称' },
          { field: 'date', displayName: '堵截案例单位' },
          { field: 'name', displayName: '堵截时间' },
          { field: 'name', displayName: '堵截案例分类' },
          { field: 'name', displayName: '涉案事项' },
          { field: 'name', displayName: '堵截案例级别' },
          { field: 'name', displayName: '堵截金额(万元)' }
        ],
        type: 'json',
        // 表题
