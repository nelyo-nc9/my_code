  ...
  <div class="main">
    <div class="header" v-show="isShowInfo==false">
      ...
      <el-breadcrumb separator-class="el-icon-arrow-right" style="font-size:12px;">
        ...
        <el-breadcrumb-item>安全事故信息管理</el-breadcrumb-item>
    ...
    <div style="margin:20px 0;" v-show="isShowInfo==false">
      ...
      <el-button style="font-size:12px;" plain @click="exportFile('安全事故信息管理')">导出</el-button>
      <el-button style="font-size:12px;" plain @click="Toprint('安全事故信息管理')">打印</el-button>
    </div>
  ...
  data() {
    return {
      ...
      tableColumn: [
        ...
        { prop: 'name', label: '直接损失金额(万元)', width: '', sortable: 'true' },
        { prop: 'name', label: '安全事故类别', width: '', sortable: 'true' },
        { prop: 'name', label: '事故级别', width: '', sortable: 'true' },
  ...
  methods: {
    ...
    Toprint(title) {
      ...
      printJS({
        ...
        properties: [
          { field: 'orgName', displayName: '事故名称' },
          { field: 'date', displayName: '事发机构' },
          { field: 'name', displayName: '事发时间' },
          { field: 'name', displayName: '直接损失金额(万元)' },
          { field: 'name', displayName: '安全事故类别' },
          { field: 'name', displayName: '事故级别' },
    ...
    exportFile(title) {
      ...
      // 字段名
      const filterVal = ['orgName', 'date', 'name', 'date', 'name', 'name', 'name', 'name', 'name', 'name', 'name', 'name']
      let list = []
      // 导出全部或导出选中
      if (this.downloadObject && this.downloadObject.length > 0) {
        list = this.downloadObject
