...
export default {
    ...
    methods: {
        ...
        ...mapActions('map2d/mapIndex', ['getRelationsInfoApiByUrl']),
        bindSymbol(features) {
            return _.forEach(features, v => {
                const { type, subType, status = '0' } = v.attributes || {};
                v.symbol = v.symbol || Object.assign({ type, subType, status }, symbolConfig.randerFeatureStyle({ type, status }));
            });
        },
        ...
        getStaticMap(param) {
            ...
            const importPartFeatures = [
                { geom: { type: 'Point', points: '0,0' }, attributes: { id: '120', isouter: false, name: '总行', type: 'importPart', subType: 'headOffice', rType: DEVICETYPE['headOffice'] }, symbol: symbolConfig.randerFeatureStyle({ type: 'camera', subType: 'boltipc', status: 1 }) }
            ];
            this.bindSymbol(importPartFeatures); // 追加绑定图标样式
            this.setFeatures({ type: 'importPart', features: importPartFeatures }); // 请求加载要素
            // 请求图层数据-摄像机点位要素
            const cameraFeatures = [
                { geom: { type: 'Point', points: '0,0' }, attributes: { id: '4', isouter: false, name: '总行', type: 'camera', subType: 'boltipc', rType: DEVICETYPE['boltipc'] }, symbol: symbolConfig.randerFeatureStyle({ type: 'camera', subType: 'boltipc', status: 1 }) }
            ];
            this.bindSymbol(cameraFeatures); // 追加绑定图标样式
            this.setFeatures({ type: 'camera', features: cameraFeatures }); // 请求加载要素

            // this.getRelationsInfoApiByUrl({
            //     id: 1
            // })
            //     .then(res => {
            //         console.log('关联关系请求成功', res);
            //     })
            //     .catch(err => {
            //         console.log(err);
            //     });
        ...
        // 单击楼层平面地图的回调方法
        mapClickEvt(evt) {
            console.log(window.$context2D.map.getView().getZoom());
            console.log('鼠标单击事件', evt);
            if (this.drawActive) { console.log(evt); return; } // 绘制状态时单击无效
            const { feature } = evt;
            // 判断是否存在feature(要素)
