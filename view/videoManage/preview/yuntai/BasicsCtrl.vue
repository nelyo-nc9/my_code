    ...
    <div class="advancedControl" v-if="showMore" ref="advancedControl">
      <div class="tit">
        ...
        <div class="title" @click='advancedOpen=!advancedOpen'>
          <i :class="!advancedOpen ? 'el-icon-arrow-up' : 'el-icon-arrow-down'"></i>
          &nbsp;高级控制
        </div>
      </div>
      <div class="advanceSelected" v-if='advancedOpen'>
        ...
        <div v-show="advanceSelected === 'yuzhiwei'" class="yuzhiwei advance-item">
          <div class="advance-box clearFloat">
            <el-select v-model="presetVal" size='mini'
            :disabled='presetSetOpen'
            style="width:140px;float:left; background: gray">
              <el-option
                v-for="item in presetAllList"
                :key="item.value"
                :label="item.label"
                :value="item.value">
              </el-option>
          ...
          <div class="advance-set" v-if='presetSetOpen'>
            <div class="setList clearFloat">
              ...
              <el-select v-model="presetAllVal" size='mini'
              style="width:120px;float:left;" transfer>
                <el-option
                  v-for="item in presetAllList"
                  :key="item.value"
                  :label="item.label"
            ...
            <div class="setList clearFloat">
              ...
              <el-select v-model="presetDelVal" size='mini'
              style="width:120px;float:left;" transfer>
                <el-option
                  v-for="item in presetDelList"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
                </el-option>
        ...
        <div v-if="advanceSelected === 'saomiao'" class="saomiao advance-item">
          <div class="advance-box clearFloat">
            <div class="advance-btn-big " @click="scanAuto" :class="{'dis-btn': scanSetOpen}">随机扫描</div>
            <div class="advance-btn-big " @click="linearScan" :class="{'dis-btn': scanSetOpen}">自动扫描</div>
...
<script>
import { YUNNAN_CTRL_SET, YUNNAN_CTRL_GET } from '@src/http/video/videoPreview.api.js'
...
export default {
  ...
  methods: {
    ...
    getYuntai() {
      ...
      const res = this.pluginStatus.source
      let devInfo = {}
      if (res) {
        if (res.sipCode) {
          // const id = window.serverId[res.shareServer]
          devInfo = {
            gbPlaDevIp: res.devIp,
            gbPlaDevPort: res.devDport,
            gbDevId: res.gbDevId,
            gbPlaNvrId: res.gbPlaNvrId
          }
        } else {
          devInfo = {
            devIp: res.devIp,
    ...
    shotInit() {
      ...
      //   .then(res => {
      //     this.$messageSuccess('镜头初始化成功！')
      //   })
      //   .catch(err => {
      //     this.$messageError('镜头初始化失败！')
      //     console.log(err)
      //   })
    },
    // 调用预置位
    transPreset() {
      ...
      this.controlDome('gotoPreset', this.speed, this.presetVal)
        .then(res => {
          this.$messageSuccess('调用预置位成功！')
        })
        .catch(err => {
          this.$messageError('调用预置位失败！')
          console.log(err)
        })
    },
    getPresets(param) { // 因为没有获取真实预设位，默认进去为[]
      // 获取预置位
    ...
    transCuise() {
      ...
      this.controlDome('runSeq', this.speed, 1, this.cruiseVal)
        .then(itme => {
          this.$messageSuccess('巡航调用成功！')
        })
        .catch(() => {
          this.$messageError('巡航调用失败！')
        })
    },
    StopCuise() {
    ...
    async saveCurisePreset() {
      ...
      for (let i = this.cruiseSetData.length - 1; i >= 0; i--) {
        ...
        await this.controlDome('fillPreSeq', item.speed, item.preset, this.cruiseRouteVal, item.time, i + 1).catch(err => {
          if (err) {
            result = false
          }
        })
        await this.controlDome('setSeqDwell', item.speed, item.preset, this.cruiseRouteVal, item.time, i + 1).catch(err => {
          if (err) {
            result = false
          }
        })
        await this.controlDome('setSeqSpeed', item.speed, item.preset, this.cruiseRouteVal, item.time, i + 1).catch(err => {
          if (err) {
            result = false
          }
        })
    ...
    startRecordTraj() {
      this.controlDome('staMemCruise', this.orbitSpeedVal, 1, this.orbitRouteVal)
        .then(() => {
          ...
          this.$messageSuccess('开始录制轨迹成功！')
        })
        .catch(err => {
          this.$messageError('开始录制轨迹失败！')
          console.log(err)
        })
    },
    stopRecordTraj() {
      this.controlDome('stoMemCruise', this.orbitSpeedVal, 1, this.orbitRouteVal)
        .then(() => {
          ...
          this.$messageSuccess('停止录制轨迹成功！')
        })
        .catch(err => {
          this.$messageError('停止录制轨迹失败！')
          console.log(err)
        })
    },
    // 扫描
    ...
    linearScan() {
      ...
      this.controlDome('linearScan')
        .then(res => {
          this.$messageSuccess('随机扫描成功！')
        })
        .catch(err => {
          this.$messageError('随机扫描失败！')
          console.log(err)
        })
    },
    leftEdge() {
      this.controlDome('setLeftBorder', this.scanSpeedVal)
        .then(res => {
          this.$messageSuccess('设置左限位成功！')
        })
        .catch(err => {
          this.$messageError('设置左限位失败！')
          console.log(err)
        })
    },
    rightEdge() {
      this.controlDome('setRightBorder', this.scanSpeedVal)
        .then(res => {
          this.$messageSuccess('设置右限位成功！')
        })
        .catch(err => {
          this.$messageError('设置右限位失败！')
          console.log(err)
        })
    },
    async controlDome(
      ...
      if (!this.$store.getters.plugin.valid) {
        this.$messageWarning('请先下载插件，或当前浏览器不支持插件！')
        return Promise.reject(new Error(false))
      }
      if (!this.pluginStatus.isPlay) { return this.$messageError('请选择设备！') }
      // const item = this.plugin.videoPageData[this.plugin.checkedPane]
      const item = this.pluginStatus.source
      if (!item) {
        return Promise.reject(new Error(false))
      }
      // if (!this.power) {
      ...
      return new Promise((resolve, reject) => {
        YUNNAN_CTRL_SET(param, item.id, item.type)
          ...
          .catch(error => {
            ...
            if (speed !== 0) {
              return this.$messageError('云台操作错误或无设备！')
            }
          })
      })
    },
