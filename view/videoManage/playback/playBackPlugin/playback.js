...
export const openPlaybackList = async(pluginDom, data, isQuery, isTime = false) => {
  ...
  if (data.isCenter) {
    ...
    if (isQuery) {
      ...
      res = videoFilter(res.data.res || res.data, data)
      if (!res.eventList.length) {
        return [{ msg: '查询无录像！' }]
      }
    }
    let startTime = isQuery ? isTime ? data.time : res.eventList[0].startTime : data.newStartTime
    // 结束时间为录像查询的最后时间  videoFilter 方法中已将录像段的最后结束时间转成查询时间 若录像段结束时间小于查询时间 就以录像段结束时间为准
    let endTime = isQuery ? res.eventList[res.eventList.length - 1].endTime : data.eventList[data.eventList.length - 1].endTime
    ...
    if (isQuery) {
      ...
      for (const iterator of data.eventList) {
        if (iterator.startTime < data.newStartTime < iterator.endTime) {
    ...
    let startTime = isQuery ? data.startTime : data.newStartTime
    // 结束时间为录像查询的最后时间  videoFilter 方法中已将录像段的最后结束时间转成查询时间 若录像段结束时间小于查询时间 就以录像段结束时间为准
    // 开流接口
    let objOpen = {
      channel: data.channel,
    ...
    let resOpen
    try {
      resOpen = await axios.post('/record/frontdevPlayBack', objOpen) // 前端回放录像查询
    } catch (error) {
      return {}
    }
...
export const getQianDuanList = async(data) => {
  ...
  console.log(res, 'queryQianduanList返回值')
  return res.data
}
// 查询前端数据列表
export const queryQianduanList = async(data) => {
  ...
  try {
    ...
    // console.log(res, 'record/frontdevSearch 返回值')
    if (res.data.recordList) {
      res.data.recordInfo = res.data.recordList.map(item => {
        ...
        return {
          ...data,
          ...item,
          startTime: cpItem.startTime,
          endTime: cpItem.endTime,
...
export const downloadVideoSegments = async(pluginDom, data, isQuery, callback) => {
  ...
  try {
    ...
    if (isQuery) {
      ...
      strmInfo = res.eventList[0].strmInfo
      startTime = res.eventList[0].startTime
      endTime = res.eventList[res.eventList.length - 1].endTime
    } else {
...
// 格式化byte,record/frontdevSearch这个接口返回值的fileSize是kb，不是byte。更正单位。
...
export const formatRecordType = (type) => {
  if (type === 200 || type === 'timeRecord' || type === 1) {
    return '定时录像'
  } else if (type === 201 || type === 'manualRecord' || type === 2) {
    return '手动录像'
  } else if (type === 'eventRecord' || type === 4) {
    return '事件录像'
  } else if (type === 9001) {
    return '智能录像'
  } else {
...
export const downloadDev = async(record, time, path) => {
  ...
  const fileName = path + '\\' + (record.channelName || record.fileName) + seTime + time + format
  let param, openParam, objOpen
  if (!record.isCenter) {
    console.log('===isnotCenter')
    // 前端需要查询
    objOpen = {
      channel: record.result.channel,
      devIp: record.result.devIp,
      devPort: record.result.devPort,
      endTime: record.result.endTime,
