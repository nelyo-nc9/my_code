<template>
  <div class="icon-box">
    ...
    <el-dialog :visible.sync="recodeFocus" class="check-log-dialog" width="50%" :before-close="cancelSub" append-to-body
        ...
        <el-table :data="tableData" max-height="250" stripe border style="width: 100%"
          ...
          <el-table-column show-overflow-tooltip :resizable="false" label="操作" width="150" align="center">
            <template slot-scope="scope">
              <i cornflowerblue @click="checkLogDetailsHandleClick(scope.row)" title="查看" :class="[scope.row.event === '截图' ? 'el-icon-picture-outline' : 'el-icon-video-camera']"></i>
    ...
    <el-dialog :visible.sync="detailInfo" :before-close="cancelSubInfo" append-to-body :close-on-click-modal="false">
      <div class="img-box" v-if="activeBox === 'img'">
        <img :src="path" alt="截图详情">
      </div>
      <div class="playback-box" v-if="activeBox === 'video'">
        <play-back-plugin ref="playBackPlugin" :defaultPane="defaultPane" :configuration="configuration"  :iconShow="iconShowParams"></play-back-plugin>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    checkLogDetailsHandleClick(data) {
      ...
      if (data.event === '截图') {
        ...
        this.$nextTick(() => {
          this.$refs.playBackPlugin.openPlayback([params])
        })
      }
    },
