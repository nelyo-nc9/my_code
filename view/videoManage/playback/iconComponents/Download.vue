<template>
  <div class="down">
    <i
      ...
      :class="{'disable': selectedObj.dumpHandle != '' || !selectedObj.isPlay}"
      title="下载"
      @click="openDownload"
    ></i>
...
export default {
  ...
  methods: {
    ...
    saveCutDown() {
      ...
      console.log(time)
      this.$emit('iconClick', 'openDownload', time)
    },
    sTimeChange(val) {
      this.startTime = this.$moment(val, 'HH:mm:ss').toDate()
      // this.startTime = this.$moment(val).format('HH:mm:ss')
    },
    eTimeChange(val) {
      this.endTime = this.$moment(val, 'HH:mm:ss').toDate()
      // this.endTime = this.$moment(val).format('HH:mm:ss').toDate()
    }
  }
}
</script>
