<template>
  <div
    class="dp-con"
    @mouseenter.stop="showPlayspeed"
    @mouseleave="hidePlayspeed"
  >
    <i
      class="icon iconfont"
      :class="[selectedObj.pitchSpeed.icon, !selectedObj.isPlay?'disable':'']"
      title="速度"
    ></i>
    <ul v-show="showSpeed && selectedObj.isPlay">
...
export default {
  ...
  methods: {
    ...
    showPlayspeed() {
      // this.showSpeed = this.selectedObj.isPlay ? true : false
      this.showSpeed = true
      this.$emit('iconClick', 'showPlayspeed', this.showSpeed)
    },
    hidePlayspeed() {
      this.showSpeed = false
      this.$emit('iconClick', 'hidePlayspeed', this.showSpeed)
    }
  }
}
</script>
