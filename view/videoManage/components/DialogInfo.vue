<!-- 树节点详细信息弹窗 -->
<template>
    <el-dialog :visible.sync="showDialogInfo" width="580px" center :before-close="closeModal" :show-close="false" :close-on-click-modal="false">
      <iframe v-if="showDialogInfo" style="width: 100%; height: 100%; position: absolute; top: 0; left: 0; border: 0;"></iframe>
      <div slot="title">
        <span class="title-span">{{nodeTypeObj[nodeType]}}</span>
        <i class="el-icon-close close-icon" @click="closeModal"></i>
      </div>
      <!-- 设备详情 -->
      <div class="box" v-if="nodeType==='equ'">
        <div class="left">
          <span>所属子系统</span>
          <span>所属机构</span>
          <span>所属重点部位</span>
        ...
        <div class="right">
          <span :title="equInfo.subSystemName">{{equInfo.subSystemName}}</span>
          <span :title="equInfo.orgName">{{equInfo.orgName}}</span>
          <span :title="equInfo.keyPartName">{{equInfo.keyPartName}}</span>
          <span>---</span>
          <span>{{equInfo.brand}}</span>
          <span>{{equInfo.type}}</span>
          <span>{{equInfo.modelNum}}</span>
          <span :title="equInfo.name">{{equInfo.name}}</span>
          <span>---</span>
          <span>{{equInfo.ip}} <i v-show="equInfo.ip" @click="pingFunc(equInfo.ip)">ping</i><i v-show="ipStatusObj[ipStatusVal]" :class="[ipStatusVal===1 ? 'green-color' : 'red-color']">{{ipStatusObj[ipStatusVal]}}</i> </span>
          <span>{{equInfo.uniqueId}}</span>
          <span>{{equInfo.assetCode}}</span>
          <span :title="equInfo.remark">{{equInfo.remark}}</span>
        </div>
      </div>
      <!-- 通道详情 -->
      <div class="box" v-if="nodeType==='res'">
        <div class="left">
          <span>所属子系统</span>
          <span>所属机构</span>
          <span>所属重点部位</span>
        ...
        <div class="right">
          ...
          <span>{{resInfo.ip}} <i v-show="resInfo.ip" @click="pingFunc(resInfo.ip)">ping</i><i v-show="ipStatusObj[ipStatusVal]" :class="[ipStatusVal===1 ? 'green-color' : 'red-color']">{{ipStatusObj[ipStatusVal]}}</i> </span>
     ...
     <!-- 重点部位详情 -->
      <div class="box" v-if="nodeType==='loc'">
        <div class="info-item" v-for="(item,index) in locLabels" :key="item.props + '_' + index">
          <div class="info-key">{{item.label}}</div>
          <div class="info-value" :title="locInfo[item.prop] || '---'">{{locInfo[item.prop] || '---'}}</div>
        </div>
      </div>
     <!-- 机构详情 -->
      <div class="box" v-if="nodeType==='org'">
        <div class="info-item" v-for="(item,index) in orgLables" :key="item.props + '_' + index">
          <div class="info-key">{{item.label}}</div>
          <div class="info-value" :title="orgInfo[item.prop] || '---'">{{orgInfo[item.prop] || '---'}}</div>
        </div>
      </div>
    </el-dialog>
</template>
...
import { getResourceInfoApi, getDeviceInfoApi, getLocOrOrgInfoApi, getIpIspingApi } from '@src/http/video/videoConfig.api.js'
...
export default {
  ...
  data() {
    return {
      ...
      statusCh: ['', '正常', '已撤销', '已合并', '临时停业'], // 机构状态
      kindArr: ['', '总行', '一级分行', '二级分行', '直管行', '县级行', '园区'], // 机构类别
      keynote: [], // 重点部位状态
      ipStatusObj: {
        1: '通',
        2: '不通'
    ...
    getDataDircColumnListApiFn() {
      ...
      queryDataDictBasicInfoApi(option).then(res => {
        ...
        if (res.data.code === 0) {
          ...
          // this[this.paramsArrOption[type]] = children.dataList
        }
        // console.log(res, '获取机构状态和机构类别============')
        // this[this.paramsArrOption[type]] = []
      }).catch(err => {
        console.log(err)
      })
    },
      ...
      if (data.tierType === 'equ') {
        ...
        getLocOrOrgInfoApi({
          ...
          if (res.data.code === 0) {
            ...
            console.log(this.keynote, 'this.keynote========')
            let status = this.keynote.filter(item => item.value == this.locInfo.status)
            if (status.length) {
              this.locInfo.status = status[0].data
            }
            console.log('=========', res)
            // this.locInfo.status = this.statusCh[this.locInfo.status]
            this.locInfo.oidName = this.locInfo.parent.name
          } else {
            this.$messageError(res.data.data)
          }
        }).catch(err => {
        ...
        getLocOrOrgInfoApi({
          ...
          if (res.data.code === 0) {
            ...
            let status = this.statusCh.filter(item => item.value == this.orgInfo.status)
            if (status.length) {
              this.orgInfo.status = status[0].label
            }
            let kindArr = this.kindArr.filter(item => item.value == this.orgInfo.kind)
            // this.orgInfo.status = this.statusCh[this.orgInfo.status]
            // this.orgInfo.kindName = this.kindArr[Number(this.orgInfo.kind)]
            if (kindArr.length) {
              this.orgInfo.kindName = kindArr[0].label
            }
