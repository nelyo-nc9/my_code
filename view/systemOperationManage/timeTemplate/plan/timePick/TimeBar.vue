<template>
  <div class="time-bar" @mouseleave="leave($event)">
    ...
    <div class="bar-box" :class="{disenable:disenable}" @mousemove="move($event)" @mousedown="down($event)" @mouseup="up($event)" @mouseleave="leave($event)">
      ...
      <div class="bar-box-f0 clearfix">
        <span class="iconfont" style=font-size:18px; @click="deleteTimeBar">&#xe638;</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    down(e) {
      if (!this.isPermissionEdit) {
        this.$messageWarn('请点击编辑按钮后再操作', '提示')
        return
      }
      ...
      if (e.target.className === 'bar-box-f3') {
        ...
        if (this.timeList.length >= this.maxNum) {
          this.$messageWarn('同一天最多支持8个不同的时间段', '提示')
...
.bar-box-f3 {
  ...
  box-sizing: border-box;
  position: relative;
  background-color:rgba(217, 230, 250, .5);
}
.bar-box-f0 {
  cursor: pointer;
  position: absolute;
