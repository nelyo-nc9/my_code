...
<script>
import { getMoreLinks, setMoreLinks } from '@src/http/protectiveCabin/channe.api.js'
...
export default {
  ...
  created() {
    ...
    this.selectedData.forEach((item) => {
      ids.push({
        id: item.id,
        ref: this.configType === '防护舱主机关联配置' ? 1 : 2,
      })
    })
  ...
  methods: {
    ...
    save() {
      ...
      this.selectedData.forEach((i) => {
        let obj = {
          id: i.id,
          type: this.configType === '防护舱主机关联配置' ? 1 : 2,
        }
        let linkages = {}
        ...
        if (hasLinks) {
          ...
          linkages.cabin = hasLinks.cabin.map((x) => x.rcId || x.id)
    ...
    async createGplot(data) {
      ...
      if (reqLink) {
        if (this.activeTabName === '1') {
          ...
          reqLink.cabin = (data ? data.children : this.rightSelectData.children) || []
        } else {
      ...
      let gpData = [
        {
          ...
          type:
            this.configType === '防护舱主机关联配置'
              ? 'videoHost'
              : this.configType === '防护舱通道关联配置'
              ? 'vidicon'
              : 'alarm',
        },
      ]
