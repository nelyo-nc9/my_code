    ...
    <div class="content">
        ...
        <div class="form-content"
          ...
          <el-form ref="searchModel"
            ...
            <el-form-item label="IP地址"
                          prop="ip">
              <el-input v-model="searchModel.ip"
                        placeholder="请输入IP地址"></el-input>
            </el-form-item>
            <el-form-item label="端口"
                          prop="port">
              <el-input v-model="searchModel.port"
                        placeholder="请输入端口"></el-input>
            </el-form-item>
            <el-form-item label="接入服务器"
                          prop="accessService">
              <el-input v-model="searchModel.accessService"
                        placeholder="请输入接入服务器"></el-input>
            </el-form-item>
            <el-form-item label="防护舱通道数量"
                          prop="cabinNumber">
              <el-input v-model="searchModel.cabinNumber"
                        placeholder="请输入防护舱通道数量"></el-input>
            </el-form-item>
            <el-form-item label="报警输入数量"
                          prop="inputNumber">
              <el-input v-model="searchModel.inputNumber"
                        placeholder="请输入报警输入数量"></el-input>
            </el-form-item>
            <el-form-item label="报警输出数量"
                          prop="outputNumber">
              <el-input v-model="searchModel.outputNumber"
                        placeholder="请输入报警输出数量"></el-input>
            </el-form-item>
            <el-form-item label="品牌"
                          prop="brand">
              <el-input v-model="searchModel.brand"
                        placeholder="请输入品牌"></el-input>
            </el-form-item>
            <el-form-item label="设备序列号"
                          prop="deviceSerise">
              <el-input v-model="searchModel.deviceSerise"
      ...
      <div class="query-table">
        ...
        <el-table stripe
                  ...
                  @select="select"
                  @select-all="select"
                  :height="dropdownStatus.searchCondition ? '326px':'640px'"
                  max-height="805"
                  style="width: 100%">
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { prop: 'name', sortable: true, label: '防护舱主机名称', width: '140' },
        { prop: 'ip', sortable: true, label: '防护舱主机IP', width: '140' },
        { prop: 'port', sortable: true, label: '端口', width: '80' },
        { prop: 'cabinNumber', sortable: true, label: '防护舱通道数量', width: '140' },
        { prop: 'inputNumber', sortable: true, label: '报警输入数量', width: '120' },
        { prop: 'outputNumber', sortable: true, label: '报警输出数量', width: '120' },
        { prop: 'brand', sortable: true, label: '品牌', width: '100' },
        { prop: 'modelNum', sortable: true, label: '型号', width: '120' },
        { prop: 'username', label: '用户名', width: '120' },
        { prop: 'password', label: '密码', width: '120' },
        { prop: 'deviceSerise', label: '设备序列号', width: '120' },
        { prop: 'assetCode', label: '资产编码', width: '120' },
        { prop: 'status', sortable: true, label: '在线状态', width: '100' },
      ...
      rules: rules.controllerConfiguration,
    }
  },
  methods: {
    // 机构/重点部位 下拉选 过滤规则
      ...
      this.$refs['searchModel'].validate((valid) => {
        if (valid) {
          ...
          let body = {
            ...
            equal: {
              cabinNumber: this.searchModel.cabinNumber,
              inputNumber: this.searchModel.inputNumber,
              outputNumber: this.searchModel.outputNumber,
            },
          }
    ...
    exportFile() {
      ...
      if (this.selectedData && this.selectedData.length > 0) {
        ...
        export_json_to_excel(tHeader, data, `防护舱主机导出${this.$moment(new Date()).format('YYYY-MM-DD HH-mm-ss')}`)
      } else {
        let params = {
          pattern: 'inquery',
          equal: {
        ...
        let name = `防护舱主机导出${this.$moment(new Date()).format('YYYY-MM-DD HH-mm-ss')}.xlsx`
        download(
          url,
          name,
          () => {},
