        ...
        <div class="form-content"
          ...
          <el-form ref="searchModel"
            ...
            <el-form-item label="型号"
                          prop='modelNum'>
              <el-input v-model="searchModel.modelNum"
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { prop: 'isVideoLink', label: '关联视频通道', width: '120' },
        { prop: 'isAudioLink', label: '关联对讲通道', width: '120' },
        { prop: 'isCabinLink', label: '关联防护舱通道', width: '120' },
        { prop: 'isOutputLink', label: '关联输出通道', width: '120' },
          ...
          lookupApi(body).then((res) => {
            if (res.data.data) {
              res.data.data.hosts.forEach((item, index) => {
                ...
                item.isCabinLink = item.isCabinLink == 2 ? '已关联' : '未关联'
    ...
    exportFile() {
      ...
      if (this.selectedData && this.selectedData.length > 0) {
        ...
        export_json_to_excel(
          tHeader,
          data,
          `防护舱主机关联配置导出${this.$moment(new Date()).format('YYYY-MM-DD HH-mm-ss')}`
        )
      } else {
        let params = {
          pattern: 'inquery',
        ...
        let name = `防护舱主机关联配置导出${this.$moment(new Date()).format('YYYY-MM-DD HH-mm-ss')}.xlsx`
        download(
          url,
          name,
          () => {},
