      ...
      <div class="content-box">
        <div class="header"
          ...
          <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>报警输入配置</el-breadcrumb-item>
          ...
          <div class="right-header-search">
            <div class="search">
              <el-input v-model="relevanceChannelDataObj.key"
                        ...
                        placeholder="请输入IP或报警输入名称"></el-input>
            </div>
          </div>
        </div>
        <div class="right-table"
          ...
          <el-table :data="inputTableData"
            ...
            <el-table-column v-for="item in inputTableColumn"
                             :key="item.label"
                             :prop="item.prop"
                             :label="item.label"
                             :width="item.width"
        ...
        <InputInfo v-if="isShowTopo"
                   :isShowTopo.sync="isShowTopo"
                   :hostInfo="selectItem[0]"
                   relatedType="1"></InputInfo>
...
export default {
  ...
  data() {
    return {
      ...
      inputTableColumn: [
        // 表格标题行内容
        { prop: 'orgName', label: '所属机构', width: '130px', sortable: 'true' },
        { prop: 'keyPartName', label: '所属重点部位', width: '160px', sortable: 'true' },
        { prop: 'deviceName', label: '所属防护舱主机', width: '160px', sortable: 'true' },
        { prop: 'name', label: '报警输入名称', width: '160', sortable: 'true' },
        { prop: 'serise', label: '报警输入号', width: '140px', sortable: 'true' },
        ...
        { prop: 'updatedBy', label: '录入人', width: '120px', sortable: 'true' },
  ...
  methods: {
    ...
    relevanceChannelList(type) {
      ...
      relevanceChannelApi(obj).then((res) => {
        ...
        this.inputTableData = res.data.data.channels
    ...
    explod() {
      if (this.explodArr && this.explodArr.length > 0) {
        ...
        let tHeader = this.inputTableColumn.map((item) => item.label)
        let filterVal = this.inputTableColumn.map((item) => item.prop)
        ...
        export_json_to_excel(
          tHeader,
          data,
          `报警输入关联设备导出${this.$moment(new Date()).format('YYYY-MM-DD HH-mm-ss')}`
        )
      } else {
        let body = {
        ...
        let url = `${window.location.protocol}//${window.location.hostname}:${window.location.port}/api/resource/v1/cabin/input/linkage/download`
        let name = `报警输入关联设备导出${this.$moment(new Date()).format('YYYY-MM-DD HH-mm-ss')}.xlsx`
        download(
          url,
          name,
          () => {},
