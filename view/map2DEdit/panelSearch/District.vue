...
export default {
  ...
  mounted() {
    // console.log('keywords="keywords"', this.keywords)
  ...
  methods: {
    ...
    handleNodeClick(areaData) {
      // this.$store.state.map2d.mapIndex.selectDistrictName = areaData.name
      let data = {
        adcode: areaData.adcode
      }
      this.getDistrictSetting(data).then(suc => {
        this.setCurrPanelToolComponent('DistrictSetting')
        this.setPanelInfoStatus(true) // 显示右侧面板
      })
      this.setNowSelectAreaInfo(areaData)
    },
    updateDivToList() {
      this.list.length = 0
  ...
  created() {
    this.setNowSelectAreaInfo(null)
