...
<template>
  <div>
    <div class="top-content">
      <div class="top-contentone">
        <div>
          <el-button size="mini" plain @click="handleDetection" :disabled="selectData.length <= 0" >
            ping检测
          </el-button>
          <!-- <el-button size="mini" plain @click="handleRepair" :disabled="selectData.length>1||selectData.length==0" >
            <span class="icon iconfont">&#xe719;</span> 报修
          </el-button> -->
          <el-button size="mini" plain @click="getList">
            刷新
          </el-button>
          <el-button size="mini" plain @click="exportFile" >导 出</el-button>
          <el-button size="mini" plain @click="Toprint">打 印</el-button>
...
<script>
import { getsubsystemListApi, getPingApi } from '@src/http/securityEquipmentControl/securityEquipmentControl.api.js'
export default {
  props: ['currentOrgId', 'currentserial', 'currentKeyPartId'],
  data() {
    return {
      // 在线数
      inLine: 0,
  ...
  methods: {
    ...
    getList() {
      ...
      this.tableData = []
      getsubsystemListApi(params).then(res => {
        let result = res.data
        if (result.code === 0) {
          this.$messageSuccess('获取设备列表成功')
          this.total = result.data.count && (result.data.count.total || 0)
          this.tableData = result.data.hosts || []
          this.inLine = result.data.count && (result.data.count.onlineTotal || 0)
          this.onLine = result.data.count && (result.data.count.offlineTotal || 0)
    ...
    formatterTable(row, column, cellValue, index) {
      // console.log('one', row, column, cellValue, index)
      let params = ''
      let obj = {}
