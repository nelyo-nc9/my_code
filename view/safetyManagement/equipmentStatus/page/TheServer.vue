  ...
  <div>
    ...
    <el-row>
      ...
      <el-button size="mini" plain @click="handleDetection" :disabled="selectData.length <= 0" >ping检测</el-button>
        <el-button size="mini" plain @click="getList">刷新</el-button>
    ...
    <div class="tableContent">
      ...
      <el-pagination style="text-align:center; margin-top:8px;" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="queryInfo.pagenum" :page-sizes="[25, 50, 100, 200]" :page-size="queryInfo.pagesize" layout="total, sizes, prev, pager, next, jumper" :total="total"></el-pagination>
    </div>
...
<script>
import { getServerApi, getPingApi, exportServerList } from '@src/http/securityEquipmentControl/securityEquipmentControl.api.js'
export default {
  data() {
    return {
      // 在线数
      ...
      tableColumn: [
        ...
        { prop: 'Device', label: '设备类型', width: 'auto', sortable: true },
        { prop: 'code', label: '设备名称', width: 'auto', sortable: true },
        { prop: 'ipAddr', label: 'IP地址', width: 'auto', sortable: true },
        { prop: 'onlineStatus', label: '在线状态', width: 'auto', sortable: true }
      ],
      // 表格数据
      tableData: [],
      // 是否在线
  ...
  methods: {
    ...
    exportFile() {
      ...
      if (this.selectData && this.selectData.length > 0) {
        ...
        // list = this.tableData
        exportServerList(params).then(res => {
          console.log(res)
        })
      }
    },
